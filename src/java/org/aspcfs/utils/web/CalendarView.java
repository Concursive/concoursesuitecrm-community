...
public void setFrontPageView(boolean frontPageView) {
        ...
        for (int i = 0; i < tmpEvents.size(); i++) {
          ...
          if ((tmpEvent != null) && (!tmpEvent.getCategory().equals("blank"))) {
            if (tmpEvent.getCategory().equals("holiday")) {
              html.append("<br>" + tmpEvent.getIcon() + "<font color=blue>" + tmpEvent.getSubject() + "</font>");
            } else {
              html.append("<br>" + tmpEvent.getIcon() + tmpEvent.getSubject());
            }
          }
        }
        tmpEvents = null;
   ...
   *@version    $Id: CalendarView.java,v 1.7 2001/07/26 19:48:10 mrajkowski Exp
   *      $
   */
