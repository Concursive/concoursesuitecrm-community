...
public class Ticket extends GenericBean {
  ...
  private int submitterContactId = -1;
  ...
  public void queryRecord(Connection db, int id) throws SQLException {
    ...
    PreparedStatement pst = db.prepareStatement(
        "SELECT t.*, " +
            ...
            "LEFT JOIN organization os ON (t.submitter_id = os.org_id) " +
  ...
  public void setSubmitterContactId(int tmp) {
    this.submitterContactId = tmp;
  }
  ...
  public int getSubmitterContactId() {
    if (submitterContactId == -1) {
      submitterId = orgId;
      submitterContactId = contactId;
    }
  ...
  /**
   * Sets the SubmitterName attribute of the Ticket object
   *
   * @param submitterName The new SubmitterName value
   */
  ...
  public boolean insert(Connection db) throws SQLException {
    ...
    try {
      ...
      sql.append(
          "INSERT INTO ticket (submitter_id, submitter_contact_id, contact_id, problem, pri_code, " +
      ...
      DatabaseUtils.setInt(pst, ++i, this.getSubmitterContactId());
  ...
  public int update(Connection db, boolean override) throws SQLException {
    ...
    if (submitterId != -1) {
      sql.append(" submitter_id = ?, ");
      sql.append(" submitter_contact_id = ?, ");
    }
    sql.append(
    ...
    if (submitterId != -1) {
      DatabaseUtils.setInt(pst, ++i, getSubmitterId());
      DatabaseUtils.setInt(pst, ++i, getSubmitterContactId());
    }
  ...
  protected void buildRecord(ResultSet rs) throws SQLException {
    ...
    submitterContactId = DatabaseUtils.getInt(rs, "submitter_contact_id");
  ...
  public String generateWebcalEvent(TimeZone tz, Timestamp created, int type) {
    ...
    if (type == TicketEventList.OPEN_TICKET) {
      webcal.append(
          "UID:www.concursive.com-ticket-alerts-" + this.getId() + CRLF);
    } else if (type == TicketEventList.OPEN_PROJECT_TICKET) {
      webcal.append(
          "UID:www.concursive.com-project-ticket-alerts-" + this.getId() + CRLF);
    }
