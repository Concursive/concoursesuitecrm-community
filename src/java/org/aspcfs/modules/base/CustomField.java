...
public class CustomField extends GenericBean implements Cloneable {
	...
	public void setParameters(HttpServletRequest request) {
		...
		if (newValue != null) {
			...
			case STATE_SELECT:
				...
				break;
				// added for Date type of fields
				// date value is filtered by prefixing 0 to month and date if it is < 10
			case DATE:
				String filteredValue = null;
				StringBuffer buffer = new StringBuffer();
				if (newValue.contains("/")) {
					StringTokenizer strToken = new StringTokenizer(newValue, "/");
					while (strToken.hasMoreTokens()) {
						filteredValue = strToken.nextToken();
						if (filteredValue.length() < 2 && Integer.parseInt(filteredValue) < 10) {
							filteredValue = "0" + filteredValue;
						}
						buffer.append(filteredValue);
						buffer.append("/");
					}
					if (buffer.length() > 0) {
						buffer.deleteCharAt(buffer.length() - 1);
					}
					enteredValue = buffer.toString();
				} else {
					enteredValue = newValue;
				}
				break;
	...
	/**
	 * Sets the randomNumber attribute of the CustomField object
	 *
	...
	/**
	 * Gets the randomNumber attribute of the CustomField object
	 *
	...
	/**
	 * Sets the defaultValue attribute of the CustomField object
	 *
	 * @param defaultValue The new randomNumber value
	 */
	...
	/**
	 * Gets the defaultValue attribute of the CustomField object
	 *
	 ...
	 */
	public String getDefaultValueString() throws Exception {
		if (this.getType() == CustomField.SELECT) {
			if (((this.getDefaultValue()) != null)) {
				if ((Integer.parseInt(this.getDefaultValue()) == -1)) {
					return null;
				}
				return ((HtmlSelect) getElementData()).getSelectedValue(getDefaultValue());
			}
			return null;
		}
		return defaultValue;
	...
	public String getHtmlElement(SystemStatus thisSystem) {
		...
		case SELECT:
			if (defaultValue != null) {
				try {
					selectedItemId = Integer.parseInt(defaultValue);
				} catch (Exception e) {
				}
			}
		...
		case DATE:
			return ("<input type=\"text\" name=\"" + elementName + "\" size=\"10\" value=\"" + StringUtils.toHtmlValue(
					enteredValue) + "\" > " +
					"<a href=\"javascript:popCalendar('details" + this.getRandomNumber() + "', '" + elementName + "','" + language + "','" + country + "');\">" +
	...
	public int insert(Connection db) throws SQLException {
		...
		"(record_id, field_id, selected_item_id, entered_number, ");
		if (!"".equals(enteredValue) && enteredValue != null) {
			sql.append("entered_value,");
		}
		if (dataId > -1) {
			sql.append("id, ");
		}
	...
	public boolean insertField(Connection db) throws SQLException {
		...
		try {
			...
			sb.append("additional_text, default_value ) VALUES (" + (id > -1 ? "?, " : "") + "?, ?, ?, ?, ?, ");
			if (entered != null) {
				sb.append("?, ");
			} else {
			...
			if (type == CURRENCY) {
				try {
					...
					for(int index = 0;index < testString.length();index++) {									
						if(StringUtils.isNumeric(testString.charAt(index))) {
					...
					Locale locale = new Locale(System.getProperty("LANGUAGE"), System.getProperty("COUNTRY"));
					formattedValue = formatCurrency(testNumber, locale);
					this.setEnteredValue(formattedValue);
					/*
					 Locale locale = new Locale(System.getProperty("LANGUAGE"), System.getProperty("COUNTRY"));
				...
				try {
					...
					if (sep == null) {
						...
						for(int i=0;i<sep.length;i++){
							if(sep[i].length()>4){
								throw new java.text.ParseException("Date must be entered in the server's local date format", 0);
						  	}
						  }
	  					Locale locale = new Locale(System.getProperty("LANGUAGE"), System.getProperty("COUNTRY"));
					...
					error = "Date must be entered in the server's local date format";
				}
			}
