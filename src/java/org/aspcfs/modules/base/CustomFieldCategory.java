...
public class CustomFieldCategory extends ArrayList {
	 ...
	 * @param noOfRecords
	 */
	public void doAggregateFunctions(Connection db, int noOfRecords, int offset) {
				...
				while (groupList.hasNext()) {
					...
					while (fieldList.hasNext()) {
						...
						if (thisField.getTypeString().equals("Number") || thisField.getTypeString().equals("Decimal Number") || thisField.getTypeString().equals("Percent") || thisField.getTypeString().equals("Currency")) {
							float value = 0;
							if (!"&nbsp;".equals(thisField.getValueHtml().trim())) {
								if (thisField.getTypeString().equals("Percent")) {
									...
									String newValue = thisField.getValueHtml().replaceAll(",", "");
									for(int index = 0;index < newValue.length();index++) {									
										if(StringUtils.isNumeric(newValue.charAt(index))) {
							...
							if (!fieldTotals.containsKey(thisField.getId())) {
								...
								fieldAverages.put(thisField.getId(), (tmpTotal + value) / recordCount);
							}
						}
					}
				}
	 ...
	 * @return total value
	 */
	public String getFieldTotal(Connection db, int fieldId) throws Exception {
		float value;
		if (fieldTotals.containsKey(fieldId)) {
			value = (Float) fieldTotals.get(fieldId);
			CustomField thisField = new CustomField(db, fieldId);
			if ("Currency".equals(thisField.getTypeString())) {
	 ...
	 */
	public String getFieldAverage(Connection db, int fieldId) throws Exception {
		float value;
