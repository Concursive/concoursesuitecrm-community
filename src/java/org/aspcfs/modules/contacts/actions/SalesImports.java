...
public final class SalesImports extends CFSModule {
    ...
    try {
      ...
      if (dependencies.size() == 0) {
        ...
        htmlDialog.setDeleteUrl(
            "javascript:window.location.href='SalesImports.do?command=Delete&importId=" + importId + "'");
      } else {
        htmlDialog.setHeader(systemStatus.getLabel("confirmdelete.header"));
        htmlDialog.addButton(
            systemStatus.getLabel("button.deleteAll"), "javascript:window.location.href='SalesImports.do?command=Delete&importId=" + importId + "'");
        htmlDialog.addButton(
  ...
  public String executeCommandDelete(ActionContext context) {
    ...
    try {
      ...
      if (recordDeleted > 0) {
        ...
        FileItem importFile = new FileItem(
            db, thisImport.getId(), Constants.IMPORT_SALES);
    ...
    context.getRequest().setAttribute(
        "refreshUrl", "SalesImports.do?command=View");
  ...
  public String executeCommandViewResults(ActionContext context) {
    ...
    String importId = context.getRequest().getParameter("importId");
    PagedListInfo pagedListInfo = this.getPagedListInfo(
        context, "SalesImportResultsInfo");
    pagedListInfo.setLink(
        "SalesImports.do?command=ViewResults&importId=" + importId);
    try {
      db = this.getConnection(context);
      ContactImport thisImport = new ContactImport(
          db, Integer.parseInt(importId));
