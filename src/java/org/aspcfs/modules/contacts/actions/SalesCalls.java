 ...
 *
 * @version $Id: SalesCalls.java,v 1.12 2002/12/23 18:27:00 chris
 *          Exp $
 * @created August 23, 2007
 */
public final class SalesCalls extends CFSModule {
  ...
  public String executeCommandView(ActionContext context) {
    ...
    if (!hasPermission(context, "sales-leads-calls-view")) {
    ...
    addModuleBean(context, "Leads", "View Activities");
    Connection db = null;
    //Pending Call List
    ...
    if (sectionId == null || pendingPagedListId.equals(sectionId)) {
      ...
      callListInfo.setLink(
          "SalesCalls.do?command=View&contactId=" + contactId +
      ...
      callList.setFollowupContactId(contactId);
    }
    ...
    if (sectionId == null || completedPagedListId.equals(sectionId)) {
      ...
      completedCallList.setFollowupContactId(contactId);
    ...
    context.getRequest().setAttribute("CallList2", callList);
  ...
  public String executeCommandSave(ActionContext context) {
    String permission = "sales-leads-calls-add";
    ...
    int tmpStatusId = -1;
    addModuleBean(context, "Leads", "Save an Activity");
    ...
    if (thisCall.getId() > 0) {
      permission = "sales-leads-calls-edit";
    }
    ...
    try {
      ...
      if (contactId != null && !"-1".equals(contactId)) {
        ...
        if (!hasPermission(context, "sales-leads-calls-add")
            || (thisContact.getOrgId() > 0 && !(hasPermission(context,
                "sales-leads-calls-add")))) {
          return ("PermissionError");
        }
        context.getRequest().setAttribute("ContactDetails", thisContact);
  ...
  public String executeCommandDetails(ActionContext context) {
    ...
    try {
      ...
      thisCalls = new Contact (db, contactId);
    ...
    context.getRequest().setAttribute("ContactDetails", thisCalls);
    return getReturn(context, "Details");
  }


  private void addFormElements(ActionContext context, Connection db) {
  ...
  public String executeCommandConfirmDelete(ActionContext context) {
    ...
    try {
      ...
      SystemStatus systemStatus = this.getSystemStatus(context);
      Contact thisContact = new Contact(db, contactId);
      if (!hasPermission(context, "sales-leads-calls-delete") || (thisContact.getOrgId() > 0 && !(hasPermission(
      ...
      if (dependencies.size() == 0) {
        ...
        htmlDialog.setDeleteUrl(
            "javascript:window.location.href='SalesCalls.do?command=Delete&contactId=" + contactId + "&id=" + id + RequestUtils.addLinkParams(
                context.getRequest(), "popup|popupType|actionId") + "'");
      } else {
        ...
        htmlDialog.addButton(
            systemStatus.getLabel("button.deleteAll"), "javascript:window.location.href='SalesCalls.do?command=Delete&contactId=" + contactId + "&id=" + id + RequestUtils.addLinkParams(
                context.getRequest(), "popup|popupType|actionId") + "'");
        htmlDialog.addButton(
  ...
  public String executeCommandDelete(ActionContext context) {
    ...
    if (recordDeleted) {
      ...
      context.getRequest().setAttribute(
          "refreshUrl", "SalesCalls.do?command=View&contactId=" + contactId + RequestUtils.addLinkParams(
  ...
  public String executeCommandLog(ActionContext context) {
    ...
    String contactId = context.getRequest().getParameter("contactId");
    addModuleBean(context, "Leads", "Log an Activity");
    Connection db = null;
    Contact thisContact = null;
    try {
      db = this.getConnection(context);
      if (contactId != null && !"".equals(contactId.trim()) && Integer.parseInt(contactId) > -1) {
        thisContact = new Contact(db, contactId);
        if (!hasPermission(context, "sales-leads-calls-add") || (thisContact.getOrgId() > 0 && !(hasPermission(
  ...
  public String executeCommandSchedule(ActionContext context) {
    ...
    String contactId = context.getRequest().getParameter("contactId");
    addModuleBean(context, "Leads", "Schedule an Activity");
    Connection db = null;
    Contact thisContact = null;
    try {
      db = this.getConnection(context);
  ...
  public String executeCommandModify(ActionContext context) {
    addModuleBean(context, "Leads", "Activities");
    ...
    try {
      ...
      thisCall = new Call(db, callId);
      thisContact = new Contact(db, contactId);
      if (!hasPermission(context, "sales-leads-calls-edit") || (thisContact.getOrgId() > 0 && !(hasPermission(
  ...
  public String executeCommandComplete(ActionContext context) {
     ...
     *  }
     */
    addModuleBean(context, "Leads", "Complete Activity");
    //Process parameters
    String contactId = context.getRequest().getParameter("contactId");
    int callId = -1;
  ...
  public String executeCommandForwardCall(ActionContext context) {
    ...
    Contact thisContact = null;
    addModuleBean(context, "Leads", "Forward Message");
  ...
  public String executeCommandCancel(ActionContext context) {
    if (!hasPermission(context, "sales-leads-calls-delete")) {
      return ("PermissionError");
    }
    addModuleBean(context, "Leads", "Cancel Activity");
    //Process parameters
    String contactId = context.getRequest().getParameter("contactId");
    int callId = -1;
    if (context.getRequest().getParameter("parentId") != null && !"".equals(
  ...
  public String executeCommandSendCall(ActionContext context) {
    ...
    String contactId = context.getRequest().getParameter("contactId");
    Contact thisContact = null;
    addModuleBean(context, "Leads", "Send Message");
    Connection db = null;
    try {
      db = this.getConnection(context);
      thisContact = new Contact(db, contactId);
      if (!hasPermission(context, "sales-leads-calls-edit")) {
        return ("PermissionError");
      }
      if (!hasAuthority(db, context, thisContact)) {
        return ("PermissionError");
