...
public final class ExternalContacts extends CFSModule {
   ...
   * @return          Description of the Return Value
   */
  public String executeCommandContactMessageDetails(ActionContext context) {
    addModuleBean(context, "External Contacts", "Contact Message Details");
    Connection db = null;
    Contact thisContact = null;
    ...
    try {
      ...
      contactMessage.setBuildMessage(true);
      contactMessage.queryRecord(db, Integer.parseInt(messageId));
      context.getRequest().setAttribute("contactMessage", contactMessage);
    } catch (Exception errorMessage) {
      context.getRequest().setAttribute("Error", errorMessage);
      return ("SystemError");
    } finally {
    ...
    context.getRequest().setAttribute("ContactDetails", thisContact);
    if (popup) {
      return ("ContactMessageDetailsPopupOK");
    }
    return ("ContactMessageDetailsOK");
  }
   ...
   * Contact object is added to the request if successful.
   *
    ...
    if (recordInserted) {
      ...
      if (context.getRequest().getParameter("popup") != null) {
        if (context.getRequest().getParameter("source") != null) {
          ...
          if ("addactivity".equals(source) || "sendMailHomePage".equals(source)) {
  ...
  public String executeCommandSendMessage(ActionContext context) {
    ...
    try {
      ...
      if (commandName != null && !"".equals(commandName) && commandName.equals("executeCommandPrepareQuickMessage")) {
        context.getRequest().setAttribute("commandName", commandName);
      if ("".equals(recipient.getPrimaryEmailAddress())) {
          recipient.setRequestItems(context);
          recipient.update(db, context);
        }
      } else {
        if ("".equals(recipient.getPrimaryEmailAddress())) {
        context.getRequest().setAttribute(
   ...
   * @return          Description of the Return Value
   */
  public String executeCommandPrepareQuickMessage(ActionContext context) {
    context.getRequest().setAttribute("commandName", "executeCommandPrepareQuickMessage");
    return (executeCommandPrepareMessage(context));
  }
}
