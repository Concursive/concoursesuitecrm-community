 ...
 *
 */
public class CampaignRunList extends ArrayList implements SyncableList {

  private final static String tableName = "campaign_run";
  ...
  /**
   *  Constructor for the CampaignRunList object
   */
  public CampaignRunList() { }
   ...
   */
  public static CampaignRun getObject(ResultSet rs) throws SQLException {
    CampaignRun campaignRun = new CampaignRun(rs);
    return campaignRun;
  }
  ...
  public PreparedStatement prepareList(Connection db) throws SQLException {
    ...
    sqlSelect.append(
        "SELECT cr.*" +
        " FROM " + DatabaseUtils.getTableName(db, tableName) + " cr" +
        " WHERE cr.id > -1 ");
  ...
  private void createFilter(Connection db, StringBuffer sqlFilter) {
    ...
    if (syncType == Constants.SYNC_INSERTS) {
      if (lastAnchor != null) {
        sqlFilter.append("AND cr.entered > ? ");
      }
      sqlFilter.append("AND cr.entered < ? ");
    }
    if (syncType == Constants.SYNC_UPDATES) {
      sqlFilter.append("AND cr.modified > ? ");
      sqlFilter.append("AND cr.entered < ? ");
      sqlFilter.append("AND cr.modified < ? ");
    }
  }
