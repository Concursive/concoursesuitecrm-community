...
import org.aspcfs.modules.base.GraphTypeList;
import org.aspcfs.modules.base.GraphType;
...
public class IceletProperty extends GenericBean {
    ...
    public void buildValueString(Connection db) throws SQLException {
        ...
        if (this.getType() != null && this.getType().equals(MINORAXIS_PARAMETER)) {
            if (this.getValue() != null && !"".equals(this.getValue()) && !"-1".equals(this.getValue())) {
                StringTokenizer minorAxisToken = new StringTokenizer(this.getValue(), ";");
                String minorAxisParam = "";
                String graphId = null;
                String graphType = null;
                String minorAxisField = null;
                StringBuffer minorAxisEdit = new StringBuffer();
                ...
                while(minorAxisToken.hasMoreTokens()){
                    minorAxisParam = minorAxisToken.nextToken();
                    graphId = minorAxisParam.substring(minorAxisParam.indexOf(":")+1);
                    minorAxisParam = minorAxisParam.substring(1, minorAxisParam.indexOf(":")-1);
                    minorAxisEdit.append("<option>");
                    StringTokenizer minorAxisFieldToken = new StringTokenizer(minorAxisParam, ",");
                    while(minorAxisFieldToken.hasMoreTokens()){
                        minorAxisField = minorAxisFieldToken.nextToken();
                        CustomField customField = new CustomField(db, Integer.parseInt(minorAxisField.trim()));
                        if (customField != null && customField.getName() != null) {
                            minorAxisEdit.append(customField.getName());
                            minorAxisEdit.append(", ");
                        }
                    }
                    minorAxisEdit.delete(minorAxisEdit.length() - 2, minorAxisEdit.length() - 1);
                    ...
                    minorAxisEdit.append(" ("+ graphType +")");
                    minorAxisEdit.append("</option>");
                }
                this.setValueString(minorAxisEdit.toString());
            }
        }
        if (this.getType() != null && this.getType().equals(RECORDRANGE_SELECT)) {
            if (this.getValue() != null && !"".equals(this.getValue()) && !"Start:Relative,12;End:Relative,0".equals(this.getValue())) {
                String firstPart = null;
                String secondPart = null;
                int rangeStartValue = 0;
                int rangeEndValue = 0;
                String rangeType = null;
                String rangeSelectString = null;
                String rangeSelectValue = this.getValue();

                StringTokenizer strToken = new StringTokenizer(rangeSelectValue, ";");

                if(strToken.countTokens() == 2) {
                    firstPart = strToken.nextToken();
                    secondPart = strToken.nextToken();

                    strToken = new StringTokenizer(firstPart,",");

                    if(strToken.countTokens() == 2) {
                        rangeType = strToken.nextToken();
                        rangeStartValue = Integer.parseInt(strToken.nextToken());
                    }

                    strToken = new StringTokenizer(secondPart,",");

                    if(strToken.countTokens() == 2) {
                        rangeType = strToken.nextToken();
                        rangeEndValue = Integer.parseInt(strToken.nextToken());
                        rangeType = rangeType.substring(rangeType.indexOf(":") + 1);
                    }
                }
                if(rangeType.equalsIgnoreCase("Relative")) {
                    if (rangeEndValue == 0) {
                        rangeSelectString = rangeStartValue + " Records Ago to Current";
                    }
                    else if (rangeEndValue > 0) {
                        rangeSelectString = rangeStartValue + " Records Ago to " + rangeEndValue;
                    }
                }
                else if(rangeType.equalsIgnoreCase("Absolute")) {
                    rangeSelectString = "From " + rangeStartValue + " To " + rangeEndValue + " Records";
                }
                this.setValueString(rangeSelectString);
            }
        }
