...
public class FolderPortlet extends GenericPortlet {
    ...
    private void buildFolderList(RenderRequest request, RenderResponse response)
        ...
        int categoryId = 0;
        ArrayList groupCountArray = new ArrayList();
        ...
        try {
            ...
            recordList.buildRecordColumns(db, category);
            int noOfRecords = 0;
            Iterator records = recordList.iterator();
            while (records.hasNext()) {
                noOfRecords++;
                CustomFieldRecord thisRecord = (CustomFieldRecord) records.next();
                thisRecord.buildColumns(db, category);
                category.setRecordId(thisRecord.getId());
                category.buildResources(db);
                Iterator groupList = category.iterator();
                groupCount = 0;
                while (groupList.hasNext()) {
                    thisGroup = (CustomFieldGroup) groupList.next();
                    groupCount++;
                    thisGroup.setRecordId(thisRecord.getId());
                    thisGroup.buildResources(db);
                    finalGroup.add(thisGroup);
                    Iterator fields = thisGroup.iterator();
                    while (fields.hasNext()) {
                        thisField = (CustomField) fields.next();
        ...
        groupCountArray.add(groupCount);
        ...
        request.setAttribute("finalGroup", finalGroup);
        request.setAttribute("groupCount", groupCountArray);
