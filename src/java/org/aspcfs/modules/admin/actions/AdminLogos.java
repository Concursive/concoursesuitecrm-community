...
public final class AdminLogos extends CFSModule {
	...
	public String executeCommandEnable(ActionContext context) {
		...
		try {
			db = this.getConnection(context);
			FileItem thisItem = new FileItem(db, Integer.parseInt(itemId), Constants.QUOTES, Constants.DOCUMENTS_QUOTE_LOGO);
	...
	public String executeCommandMarkDefault(ActionContext context) {
		...
		try {
			db = this.getConnection(context);
      FileItem.updateDefaultRecord(db, Constants.DOCUMENTS_QUOTE_LOGO, Constants.QUOTES, Integer.parseInt(itemId));
		} catch (Exception e) {
			context.getRequest().setAttribute("Error", e);
			return ("SystemError");
		} finally {
