...
public final class ProcessCalculation extends CFSModule {
  ...
  public final static int OFFICE_PROVIDER_DETAILS = 1;
  public final static int OFFICE_PAYOR_DETAILS = 2;
  ...
  public String executeCommandDefault(ActionContext context) {
    ...
    try {
      ...
      providerTransDetails = new CustomFieldCategory(db, PROVIDER_TRANSACTION_DETAILS);
      ...
      officeTransDetails = new CustomFieldCategory(db, OFFICE_TRANSACTION_DETAILS);
      ...
      //build
      providerTransDetails.buildResources(db);      
      officeTransDetails.buildResources(db);   
      ...
      providerCategory.buildResources(db);
      ...
      providerRecords.buildList(db);
      providerRecords.buildRecordColumns(db, providerCategory);
      ...
      payorCategory.buildResources(db);
      ...
      payorRecords.buildList(db);
      payorRecords.buildRecordColumns(db, payorCategory);
      ...
      while (i.hasNext()) {
        ...
        if (providerOrgMapping.get(thisRec.getTaxId()) == null) {
          hasErrors = true;
          errors.put(thisRec.getTransactionId(), new String("Error: Provider with tax ID = " + thisRec.getTaxId() + " not found in CFS!"));
        } else if (!payorOrgMapping.containsKey(thisRec.getPayerId())) {
          hasErrors = true;
          errors.put(thisRec.getTransactionId(), new String("Error: Payor with ID = " + thisRec.getPayerId() + " not found in CFS!"));           
        }
        ...
        if (!hasErrors) {
          ...
          if (!(tempArray.contains(((Integer)providerOrgMapping.get(thisRec.getTaxId()))))) {
            hasErrors = true;
            errors.put(thisRec.getTransactionId(), new String("Error: Payor with ID = " + thisRec.getPayerId() + " not associated with Provider " + thisRec.getTaxId() + " in CFS!"));                 
          }
        }
        ...
        if (!hasErrors) {
          ...
          if (!tempFir.process(thisRec)) {
            hasErrors = true;
            errors.put(thisRec.getTransactionId(), new String("Error: Transaction Type " + thisRec.getType() + " not recognized"));                 
          }
        }            
      ...
      while (payorIterator.hasNext()) {
        ...
        if (!hasErrors) {
          ...
          officeRecords.buildList(db);
          officeRecords.buildRecordColumns(db, officeTransDetails);
          ...
          if (foundId > -1) {
            ...
            //update
            officeTransDetails.update(db);
          } else {
            //insert
            officeTransDetails.insert(db);
          }
        }        
      }
      ...
      while (it.hasNext()) {
        ...
        providerTransDetails.insert(db);          
      }
    } catch (Exception e) {
      errorMessage = e;
      e.printStackTrace();
    ...
    sb.append(recordList.size() + " record(s) processed<br>");
    sb.append(errors.size() + " rejected<br><br>");
    
    Iterator end = errors.keySet().iterator();
    while (end.hasNext()) {
      String k = (String)end.next();
