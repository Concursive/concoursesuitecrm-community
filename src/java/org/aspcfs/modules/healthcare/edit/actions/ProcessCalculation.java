        ...
        if (providerOrgMapping.get(thisRec.getTaxId()) == null) {
          ...
          errors.put(
              new Integer(thisRec.getId()), new String(
                  "Error: Provider with tax ID = " + thisRec.getTaxId() + " not found in Centric CRM!"));
        } else if (!payorOrgMapping.containsKey(thisRec.getPayerId())) {
          hasErrors = true;
          errors.put(
              new Integer(thisRec.getId()), new String(
                  "Error: Payor with ID = " + thisRec.getPayerId() + " not found in Centric CRM!"));
        }
        if (!hasErrors) {
          //error checking
          ArrayList tempArray = (ArrayList) payorOrgMapping.get(
          ...
          if (!(tempArray.contains(
            ...
            errors.put(
                new Integer(thisRec.getId()), new String(
                    "Error: Payor with ID = " + thisRec.getPayerId() + " not associated with Provider " + thisRec.getTaxId() + " in Centric CRM!"));
          }
        }
        if (!hasErrors) {
