...
public class ProductOptionValues extends GenericBean {
  ...
  public boolean insert(Connection db) throws SQLException {
    ...
    sql.append(
        ...
        "recurring_currency, recurring_amount, recurring_type, " + DatabaseUtils.addQuotes(db, "value")+ ", multiplier, range_min, range_max, cost_currency, cost_amount ) ");
    sql.append(
        "VALUES (" + (id > -1 ? "?, " : "") + "?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? )");
    int i = 0;
  ...
  public int update(Connection db) throws SQLException {
    ...
    sql.append(
        ...
        "recurring_amount = ?, recurring_type = ?, " + DatabaseUtils.addQuotes(db, "value")+ " = ?, multiplier = ?, " +
