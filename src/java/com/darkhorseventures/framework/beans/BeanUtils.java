 ...
 *  matching method name/form element pairs if they exist. Using Reflection, if
 *  the setXXX() method exists in the bean and a matching XXX parameter in the
 *  HttpServletRequest is found, it calls the setXXX() method with the value of
 *  the request parameter. Alternatively, this class supports nested population
 *  of objects, so if the object to be populated contains an object, it is
 *  possible to populate that object automatically. For example, class A has an
 *  instance of class B called bClass. For the auto population to populate the
 *  bClass instance, you would have <input type="text" name="bClass_Name">. The
 *  _ indicates the use of a nested class to be populated. The _ was chosen
 *  because when using JavaScript and DOM (client-side), the use of a . can
 *  cause some problems for JavaScript to work with specific elements. There is
 *  a fix for this..but I have not implemented it at this time. So, what happens
 *  in the above case is the populateObject() method will call
 *  getBClass().setName(value) instead of setName(value). This way, the nested
 *  object gets populated as well. This is probably used in the case where you
 *  want to have the ability to "pass thru" a bean to an underlying entity, so
 *  that you don't have to write the getter/setter methods twice. This means if
 *  you have an entity class such as Address that contains name, street, city,
 *  zip and state fields, with getter/setter methods in that class, but you
 *  don't want to recode the getter/setter methods in the bean class. By using
 *  nested autopopulation, you can "pass thru" to the entity and any sub-classes
 *  that entity contains. As a last note, the GenericBean class in this
 *  framework has two reference instances it maintains. One is entity, with a
 *  getEntity() and setEntity() method. The other is ejbRef with getEjbRef() and
 *  setEjbRef() methods. You don't need to duplicate these methods and instances
 *  in your own descendant bean classes, unless you do not descend from
 *  GenericBean. One last thing about this class. Notice it implements Populate.
 *  This interface allows you to implement your own populate class for special
...
public class BeanUtils
  ...
  public void populateObject(Object bean, HttpServletRequest request, String nestedAttribute, String indexAttribute) {
    ...
    while (e.hasMoreElements()) {
      ...
      if (paramValues.length > 1) {
        populateParameter(paramName, paramValues, bean, nestedAttribute, indexAttribute);
      } else {
        populateParameter(paramName, paramValues[0], bean, nestedAttribute, indexAttribute);
      }
    }
  }
  ...
  private static void populateParameter(String param, Object value, Object target, String nestedAttribute, String indexAttribute) {
    ...
    try {
      int dotPos = param.indexOf(nestedAttribute);
      int index = -1;
      // -1 = no index value. 0 or > = index number has been specified

      if (dotPos < 0) {
        int indexPos = param.indexOf(indexAttribute);
        Class[] argTypes = null;
        ...
        if (indexPos > dotPos) {
          ...
          try {
            index = Integer.parseInt(param.substring(indexPos + 1));
          } catch (Exception e) {
            // no index after all
          }
        }
        ...
        if (indexPos >= 0) {
          param = param.substring(0, indexPos);
        }
        ...
        String nestedMethod = param.substring(0, dotPos);
        ...
        populateParameter(param.substring(dotPos + 1, param.length()), value, nestedTarget, nestedAttribute, indexAttribute);
      }
    } catch (Throwable t) {
    }
  }
