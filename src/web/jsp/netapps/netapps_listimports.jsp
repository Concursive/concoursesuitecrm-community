...
<jsp:useBean id="NetworkApplicationsImportListInfo" class="org.aspcfs.utils.web.PagedListInfo" scope="session"/>
<jsp:useBean id="ExpirationContractListInfo" class="org.aspcfs.utils.web.PagedListInfo" scope="session"/>
<%@ include file="../initPage.jsp" %>
<%-- Initialize the drop-down menus --%>
<%@ include file="../initPopupMenu.jsp" %>
<%@ include file="netapps_listimports_menu.jsp" %>
...
<tr>
<td>
<a href="NetworkApplicationsImports.do">Import</a> > 
View Imports
</td>
</tr>
</table>
...
<dhv:permission name="netapps_expiration_contracts_imports-add">
  <a href="NetworkApplicationsImports.do?command=New">New Import</a><br>
</dhv:permission><br>
<table width="100%" border="0">
  <tr>
    <form name="listView" method="post" action="NetworkApplicationsImports.do?command=View">
    <td align="left">
      <select size="1" name="listView" onChange="javascript:document.forms[0].submit();">
        <option <%= NetworkApplicationsImportListInfo.getOptionValue("all") %>>All Imports</option>
        <option <%= NetworkApplicationsImportListInfo.getOptionValue("my") %>>My Imports</option>
      </select>
    </td>
    <td>
...
<table cellpadding="4" cellspacing="0" border="0" width="100%" class="pagedList">
  <tr>
    ...
    <th rowspan="2" valign="middle" width="100%">
      <strong><a href="NetworkApplicationsImports.do?command=View&column=m.name">Name</a></strong>
      <%= NetworkApplicationsImportListInfo.getSortIcon("m.name") %>
    </th>
    <th rowspan="2" valign="middle">
    ...
    <th rowspan="2" valign="middle" nowrap>
      <strong><a href="NetworkApplicationsImports.do?command=View&column=m.entered">Entered</a></strong>
      <%= NetworkApplicationsImportListInfo.getSortIcon("m.entered") %>
    </th>
    <th rowspan="2" valign="middle">
...
%>
  <tr class="row<%= rowid %>">
    ...
    <td width="100%" nowrap>
      <a href="NetworkApplicationsImports.do?command=Details&importId=<%= thisImport.getId() %>"><%= toHtmlValue(thisImport.getName()) %></a>
      <dhv:evaluate if="<%= !thisImport.canProcess() && thisImport.getFile().hasVersion(Import.ERROR_FILE_VERSION) %>">
      &nbsp;<%= thisImport.getFile().getImageTag() %><br />[<a href="javascript:window.location.href='NetworkApplicationsImports.do?command=Download&importId=<%= thisImport.getId() %>&fid=<%= thisImport.getFile().getId() %>&ver=<%= Import.ERROR_FILE_VERSION %>';">Download Error File</a>]
      </dhv:evaluate>
    </td>
    <td>
...
<br />
<dhv:pagedListControl object="NetworkApplicationsImportListInfo" tdClass="row1"/>
