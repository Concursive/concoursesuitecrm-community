...
<jsp:useBean id="roleList" class="org.aspcfs.modules.admin.base.RoleList"
             scope="request"/>
<jsp:useBean id="httpUserListInfo" class="org.aspcfs.utils.web.PagedListInfo"
             scope="session"/>
<jsp:useBean id="selectedUsers" class="java.util.ArrayList" scope="request"/>
<%@ include file="../initPage.jsp" %>
<script language="Javascript">
  function submitForm(form, access) {
    form.action = 'AdminHttpAPIManager.do?command=SetHttpAccess&access=' + access;
    form.submit();
  }
</script>
<%
...
<table class="trails" cellspacing="0">
  <tr>
    <td>
      ...
      <dhv:label name="admin.config.httpApiManager">HTTP-XML API User
        Manager</dhv:label>
    </td>
  </tr>
</table>
<%-- End Trails --%>
<form name="httpUserForm" method="post"
      action="AdminHttpAPIManager.do?command=ShowUsers">
...
<table cellpadding="4" cellspacing="0" border="0" width="100%"
  ...
  <tr>
    ...
    <th nowrap>
      <b><a href="AdminHttpAPIManager.do?command=ShowUsers&column=c.namelast">
        <dhv:label name="contacts.name">Name</dhv:label></a></b>
      <%= httpUserListInfo.getSortIcon("c.namelast") %>
    </th>
    <th nowrap>
      <b><a href="AdminHttpAPIManager.do?command=ShowUsers&column=a.username">
        <dhv:label name="accounts.Username">Username</dhv:label></a></b>
      <%= httpUserListInfo.getSortIcon("a.username") %>
    </th>
    <th nowrap>
      <b><a href="AdminHttpAPIManager.do?command=ShowUsers&column=r.role">
        <dhv:label name="project.role">Role</dhv:label></a></b>
      <%= httpUserListInfo.getSortIcon("r.role") %>
    </th>
    <th nowrap>
    ...
    <th nowrap>
      <b><dhv:label name="admin.config.httpApiAccess">HTTP-API
        Access</dhv:label></b>
    </th>
  </tr>
  <%
...
<br/>
<input type="button"
       value="<dhv:label name="button.enable.httpapiaccess">Enable HTTP-API Access</dhv:label>"
       onClick="javascript:submitForm(this.form, true);">
<input type="button"
       value="<dhv:label name="button.disable.httpapiaccess">Disable HTTP-API Access</dhv:label>"
