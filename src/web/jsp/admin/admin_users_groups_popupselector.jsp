...
<jsp:useBean id="baseList" class="org.aspcfs.modules.admin.base.UserGroupList" scope="request"/>
...
<input type="hidden" name="userId" value="<%= (thisUser != null? String.valueOf(thisUser.getId()):"-1") %>"/>
<input type="hidden" name="type" value="<%= type %>"/>
<input type="hidden" name="campaignId" value="<%= (campaignId != null? campaignId:"-1") %>" />
<br />
<center><%= UserGroupSelectorInfo.getAlphabeticalPageLinks("setFieldSubmit","elementListView") %></center>
...
<%
  ...
  if ( j.hasNext() ) {
    ...
    while (j.hasNext()) {
      count++;
      rowid = (rowid != 1?1:2);
      UserGroup thisElt = (UserGroup)j.next();
