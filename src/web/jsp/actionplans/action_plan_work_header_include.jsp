...
<%@ page import="org.aspcfs.modules.actionplans.base.*" %>
    ...
    <td>
      <table border="0" cellpadding="0" cellspacing="0" width="100%">
        <tr>
          ...
          %>
          <td nowrap
              class="stageComplete"><%= toHtml(phaseItem.getPhaseName()) %>
          </td>
          <%
          } else if (phaseItem.getActionPhaseId() == actionPlanWork.getCurrentPhaseId()) {
          %>
          <td nowrap
              class="stageCurrent"><%= toHtml(phaseItem.getPhaseName()) %>
          </td>
          ...
          %>
          <td nowrap
              class="stageIncomplete"><%= toHtml(phaseItem.getPhaseName()) %>
          </td>
          <%
              }
            }
          ...
          <td nowrap class="stageActions" align="right" width="100%">
            &nbsp;
            <dhv:evaluate
                if="<%= User.getUserRecord().getId() == actionPlanWork.getManagerId() %>">
              <dhv:label name="actionPlan.actions.colon">Actions:</dhv:label>
              <a href="javascript:displayMenu('select1','menuDisplay');"
                 onMouseOver="over(0, 1)"
                 onmouseout="out(0, 1); hideMenu('menuDisplay');"><img
                  src="images/select.gif" name="select1" id="select1"
...
<tr class="containerBody">
  <td nowrap class="formLabel">
    <dhv:label name="accounts.accounts_contacts_calls_list.AssignedTo">Assigned
      To</dhv:label>:
  </td>
  <td nowrap>
...
<dhv:evaluate if="<%= hasText(actionPlanWork.getLinkItemName()) %>">
  <tr class="containerBody">
    ...
    <td>
      <a href="Accounts.do?command=Details&orgId=<%=actionPlanWork.getOrganization().getId()%>"><%= toHtml(actionPlanWork.getLinkItemName()) %>
      </a>
...
<dhv:evaluate if="<%= actionPlanWork.getLead() != null %>">
  <dhv:evaluate
      if="<%= hasText(actionPlanWork.getLead().getAccountNumber()) %>">
    <tr class="containerBody">
      <td nowrap class="formLabel">
  ...
  <dhv:evaluate
      if="<%= actionPlanWork.getLead().getPrimaryAddress() != null %>">
    <tr class="containerBody">
      <td nowrap class="formLabel">
        <dhv:label name="accounts.accounts_add.City">City</dhv:label>,
        <dhv:label name="accounts.accounts_add.Zip">Zip</dhv:label>:
      </td>
      <td>
  ...
  <dhv:evaluate if="<%= actionPlanWork.getLead().getPotential() > 0 %>">
    <tr class="containerBody">
      <td nowrap class="formLabel">
        <dhv:label name="actionPlan.totalWkPotential">Total Wk.
          Potential</dhv:label>:
      </td>
      <td>
        <zeroio:currency value="<%= actionPlanWork.getLead().getPotential() %>"
...
<dhv:evaluate if="<%= actionPlanWork.getContact() != null %>">
  <dhv:evaluate
      if="<%= hasText(actionPlanWork.getContact().getAccountNumber()) %>">
    <tr class="containerBody">
      <td nowrap class="formLabel">
        <dhv:label name="actionPlan.prospect">Prospect</dhv:label> #:
      </td>
  ...
  <dhv:evaluate
      if="<%= actionPlanWork.getContact().getPrimaryAddress() != null %>">
    <tr class="containerBody">
      <td nowrap class="formLabel">
        <dhv:label name="accounts.accounts_add.City">City</dhv:label>,
        <dhv:label name="accounts.accounts_add.Zip">Zip</dhv:label>:
  ...
  <dhv:evaluate if="<%= actionPlanWork.getContact().getPotential() > 0 %>">
    <tr class="containerBody">
      ...
      <td>
        <zeroio:currency
            value="<%= actionPlanWork.getContact().getPotential() %>"
            code='<%= applicationPrefs.get("SYSTEM.CURRENCY") %>'
            locale="<%= User.getLocale() %>" default="&nbsp;"/>
      </td>
    </tr>
...
<dhv:evaluate if="<%= actionPlanWork.getOrganization() != null %>">
  <dhv:evaluate
      if="<%= hasText(actionPlanWork.getOrganization().getAccountNumber()) %>">
    <tr class="containerBody">
      <td nowrap class="formLabel">
        <dhv:label name="actionPlan.prospect">Prospect</dhv:label> #:
      </td>
  ...
  <dhv:evaluate
      if="<%= actionPlanWork.getOrganization().getPrimaryAddress() != null %>">
    <tr class="containerBody">
      <td nowrap class="formLabel">
        <dhv:label name="accounts.accounts_add.City">City</dhv:label>,
        <dhv:label name="accounts.accounts_add.Zip">Zip</dhv:label>:
  ...
  <dhv:evaluate if="<%= actionPlanWork.getOrganization().getPotential() > 0 %>">
    <tr class="containerBody">
      ...
      <td>
        <zeroio:currency
            value="<%= actionPlanWork.getOrganization().getPotential() %>"
            code='<%= applicationPrefs.get("SYSTEM.CURRENCY") %>'
            locale="<%= User.getLocale() %>" default="&nbsp;"/>
      </td>
    </tr>
...
<tr class="containerBody">
  ...
  <td>
    <zeroio:tz timestamp="<%= actionPlanWork.getEntered() %>"
               timeZone="<%= User.getUserRecord().getTimeZone() %>"
               dateOnly="true"/>
  </td>
</tr>
<tr class="containerBody">
      ...
      <tr class="containerBody">
        ...
        <td nowrap valign="top" class="row1" style="border-right:0px">
          <dhv:evaluate
            ...
            <a class="rollover"
               href="javascript:updateGlobalStatus('<%= !thisItemWork.isComplete() ? thisItemWork.getActionRequired() : false %>','<%= actionPlanWork.getId() %>', '<%= thisItemWork.getActionId() %>', '<%= thisItemWork.getId() %>');"><%= thisItemWork.getStatusGraphicTag(systemStatus) %>
            </a>
            <a class="rollover"
               href="javascript:updateGlobalStatus('<%= !thisItemWork.isComplete() ? thisItemWork.getActionRequired() : false %>','<%= actionPlanWork.getId() %>', '<%= thisItemWork.getActionId() %>', '<%= thisItemWork.getId() %>');"><%= toHtml(thisItemWork.getStepDescription()) %>
            </a>
          </dhv:evaluate>
          <dhv:evaluate
              if="<%= ! thisItemWork.userHasPermission(User.getUserRecord().getId(), request) %>">
