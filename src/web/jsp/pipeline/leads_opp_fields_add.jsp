...
<dhv:evaluate if="<%= (!Category.getAllowMultipleRecords()) %>">
  <a href="LeadsFolders.do?command=Fields&headerId=<%=OpportunityHeader.getId()%>&catId=<%= Category.getId() %>"><dhv:label name="accounts.accounts_fields.FolderRecordDetails">Folder Record Details</dhv:label></a> >
</dhv:evaluate>
...
<dhv:container name="opportunities" selected="folders" object="OpportunityHeader" param="<%= "id=" + OpportunityHeader.getId() %>" appendToUrl="<%= addLinkParams(request, "popup|popupType|actionId") %>">
  ...
  <dhv:evaluate if="<%= !Category.isEmpty() %>">
    <br>
    <input type="submit" value="<dhv:label name="global.button.save">Save</dhv:label>" onClick="javascript:this.form.action='LeadsFolders.do?command=InsertFields&headerId=<%= OpportunityHeader.getId() %>&catId=<%= Category.getId() %>'" />
