...
<a href="LeadsQuotes.do?command=Details&quoteId=<%= quoteBean.getId() %>&version=<%= (version!=null && !"".equals(version))?version:"" %>"><dhv:label name="quotes.quoteDetails">Quote Details</dhv:label></a> > 
...
<dhv:container name="opportunities" selected="quotes" object="opportunityHeader" param='<%= "id=" + opportunityHeader.getId() %>' appendToUrl='<%= addLinkParams(request, "viewSource") %>'>
  <dhv:container name="opportunitiesQuotes" selected="details" object="quoteBean" param='<%= "quoteId=" + quoteBean.getId() + "|version=" + version %>' appendToUrl='<%= addLinkParams(request, "viewSource") %>'>
    ...
    <input type="button" value="<dhv:label name="button.cancel">Cancel</dhv:label>" onClick="javascript:window.location.href='LeadsQuotes.do?command=Details&version=<%= version %>&quoteId=<%= quoteBean.getId() %>&orgId=<%= OrgDetails.getOrgId() %><%= addLinkParams(request, "viewSource") %>';"/>
    <br />
    ...
    <br />
    <input type="submit" value="<dhv:label name="global.button.update">Update</dhv:label>" />
    <input type="button" value="<dhv:label name="global.button.cancel">Cancel</dhv:label>" onClick="javascript:window.location.href='LeadsQuotes.do?command=Details&version=<%= version %>&quoteId=<%= quoteBean.getId() %>&orgId=<%= OrgDetails.getOrgId() %><%= addLinkParams(request, "viewSource") %>';"/>
  </dhv:container>
</dhv:container>
</form>
</body>
