...
<table cellpadding="0" cellspacing="0" width="100%" border="1" rules="cols">
  <tr class="section">
    ...
    <dhv:evaluate if="<%= FileFolder.getParentId() != -1 %>">
      <a href="LeadsDocumentsFolders.do?command=SaveMove&headerId=<%= opportunityHeader.getId() %>&id=<%= FileFolder.getId() %>&popup=true&folderId=0&return=LeadsDocuments&param=<%= opportunityHeader.getId() %>&param2=<%= FileFolder.getParentId() %><%= addLinkParams(request, "viewSource") %>"><dhv:label name="accounts.accounts_documents_file_move.Home">Home</dhv:label></a>
    </dhv:evaluate>
    <dhv:evaluate if="<%= FileFolder.getParentId() == -1 %>">
...
%>    
  <tr class="row<%= rowid %>">
    ...
    <% if(FileFolder.getParentId() != thisFolder.getId() && (FileFolder.getId() != thisFolder.getId())){  %>
      <a href="LeadsDocumentsFolders.do?command=SaveMove&headerId=<%= opportunityHeader.getId() %>&id=<%= FileFolder.getId() %>&popup=true&folderId=<%= thisFolder.getId() %>&return=LeadsDocuments&param=<%= opportunityHeader.getId() %>&param2=<%= FileFolder.getParentId() %><%= addLinkParams(request, "viewSource") %>"><%= toHtml(thisFolder.getSubject()) %></a>
