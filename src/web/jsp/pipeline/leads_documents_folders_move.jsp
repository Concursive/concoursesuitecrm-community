...
<table cellpadding="0" cellspacing="0" width="100%" border="1" rules="cols">
  <tr class="section">
    ...
    <dhv:evaluate if="<%= FileFolder.getParentId() != -1 %>">
      <a href="LeadsDocumentsFolders.do?command=SaveMove&headerId=<%= opportunityHeader.getId() %>&id=<%= FileFolder.getId() %>&popup=true&folderId=0&return=LeadsDocuments&param=<%= opportunityHeader.getId() %>&param2=<%= FileFolder.getParentId() %><%= addLinkParams(request, "viewSource") %>">Home</a>
    </dhv:evaluate>
    <dhv:evaluate if="<%= FileFolder.getParentId() == -1 %>">
      Home
      (current folder)
