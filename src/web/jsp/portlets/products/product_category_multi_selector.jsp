...
<jsp:useBean id="categoryNames" class="java.lang.String" scope="request" />
<jsp:useBean id="categoryListIds" class="java.lang.String" scope="request" />
...
<% if(!"-1".equals(categoryListIds)){ %> 
<script type="text/javascript">
<!--
	<% if(categoryListIds.equals("")){%>
	 opener.clearProductCategory();
	<%}else{%> 
	 opener.setProductCategory('<%=StringUtils.jsStringEscape(categoryNames)%>','<%=categoryListIds%>');
	 <%}%>
	 window.close();
//-->
</script>
...
<form name="categoryTree" action="Portal.do?command=SaveListProductMultiCategories&popup=true" onsubmit="javascript:return true;" method="post">
