...
<form name="details" action="TroubleTickets.do?command=Update&auto-populate=true<%= (defectCheck != null && !"".equals(defectCheck.trim()) ?"&defectCheck="+defectCheck:"") %><%= isPopup(request)?"&popup=true":"" %>" method="post">
...
<%if(defectCheck != null && !"".equals(defectCheck.trim())) {%>
  ...
  <a href="TroubleTickets.do?command=Details&id=<%= TicketDetails.getId() %>&defectCheck=<%= defectCheck %>"><dhv:label name="tickets.details">Ticket Details</dhv:label></a> >
<%}else{%>
