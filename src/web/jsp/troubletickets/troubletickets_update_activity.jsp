...
<dhv:container name="tickets" selected="activitylog" object="ticketDetails" param="<%= param1 %>">
  ...
  <%}else{ %>
    <input type="button" value="<dhv:label name="global.button.cancel">Cancel</dhv:label>" onClick="window.location.href='TroubleTicketActivityLog.do?command=View&id=<%=ticketDetails.getId()%>&formId=<%=activityDetails.getId()%>';this.form.dosubmit.value='false';" />
  <%}%>
  <br />
  <dhv:formMessage />
