...
<jsp:useBean id="SeverityList" class="org.aspcfs.utils.web.LookupList"
             scope="request"/>
<jsp:useBean id="SubmitterList" class="org.aspcfs.utils.web.LookupList"
             scope="request"/>
<jsp:useBean id="EscalationList" class="org.aspcfs.utils.web.LookupList"
             scope="request"/>
<jsp:useBean id="TicketTypeSelect" class="org.aspcfs.utils.web.HtmlSelect"
             scope="request"/>
<jsp:useBean id="User" class="org.aspcfs.modules.login.beans.UserBean"
             scope="session"/>
<jsp:useBean id="TicListInfo" class="org.aspcfs.utils.web.PagedListInfo"
             scope="session"/>
<jsp:useBean id="userGroup" class="org.aspcfs.modules.admin.base.UserGroup"
             scope="request"/>
...
<script type="text/javascript">
  ...
  function clearSiteSpecificInfo() {
    ...
    document.forms['searchTicket'].searchcodeSubmitterOrgId.value = "-1";
    changeDivContent('changeaccountsubmitter', label('label.all', 'All'));
  ...
  function clearAccountSubmitter() {
    document.forms['searchTicket'].searchcodeSubmitterOrgId.value = "-1";
    changeDivContent('changeaccountsubmitter', label('label.all', 'All'));
  }
...
<form name="searchTicket" action="TroubleTickets.do?command=SearchTickets"
      method="post" onSubmit="javascript:return checkForm(this);">
<%-- Trails --%>
...
<tr>
  ...
  <td>
    <input type="text" size="10" name="searchcodeId"
           value="<%= TicListInfo.getSearchOptionValue("searchcodeId") %>">
  </td>
</tr>
<tr>
  <td class="formLabel">
  ...
  <td>
    <input type="text" size="40" name="searchDescription"
           value="<%= TicListInfo.getSearchOptionValue("searchDescription") %>">
  </td>
</tr>

<tr>
  <td class="formLabel">
    <dhv:label name="accounts.submitter">Submitter</dhv:label>
  </td>
  <td>
      ...
      <tr>
        ...
        <td>
          <input type="hidden" name="searchcodeOrgId" id="searchcodeOrgId"
                 value="<%= TicListInfo.getSearchOptionValue("searchcodeOrgId") %>">
          &nbsp;[<a
            href="javascript:popAccountsListSingle('searchcodeOrgId','changeaccount', 'showMyCompany=true&siteId='+getSiteId()+'&filters=all|my|disabled');"><dhv:label
            name="accounts.accounts_add.select">Select</dhv:label></a>]
          &nbsp;[<a href="javascript:clearAccount();"><dhv:label
...
<tr>
  <td class="formLabel">
    <dhv:label name="accounts.Organization">Organization</dhv:label>
  </td>
  <td>
    <table cellspacing="0" cellpadding="0" border="0" class="empty">
      <tr>
        <td>
          <div id="changeaccountsubmitter">
            <%if ("".equals(TicListInfo.getSearchOptionValue("searchcodeSubmitterOrgId")) || "-1".equals(TicListInfo.getSearchOptionValue("searchcodeSubmitterOrgId"))) { %>
            <dhv:label name="quotes.all">All</dhv:label>
            <% } else { %>
            <%= toHtmlValue(SubmitterOrgDetails.getName()) %>
            <% } %>
          </div>
        </td>
        <td>
          <input type="hidden" name="searchcodeSubmitterOrgId"
      ...
      <tr>
        <td>
          <div id="changeowner">
            ...
            <% } else { %>
            <dhv:username
                id='<%= TicListInfo.getSearchOptionValue("searchcodeAssignedTo") %>'/>
            <% } %>
          </div>
        </td>
        <td>
          <input type="hidden" name="searchcodeAssignedTo" id="ownerid"
                 value="<%= TicListInfo.getSearchOptionValue("searchcodeAssignedTo") %>">
          &nbsp;[<a
            href="javascript:popContactsListSingle('ownerid','changeowner', 'listView=employees&usersOnly=true&siteId='+getSiteId()+'&reset=true');"><dhv:label
            name="accounts.accounts_contacts_validateimport.ChangeOwner">Change
          Owner</dhv:label></a>]
          &nbsp;[<a href="javascript:clearOwner();"><dhv:label
            name="button.clear">Clear</dhv:label></a>]
        </td>
      </tr>
    </table>
  ...
  <td>
    <table class="empty">
      <tr>
        ...
        <td>
          <input type="hidden" name="searchcodeUserGroupId"
                 id="searchcodeUserGroupId"
                 value="<%= TicListInfo.getSearchOptionValue("searchcodeUserGroupId") %>"/>
          [<a
            href="javascript:popUserGroupsListSingle('searchcodeUserGroupId','changegroup', 'siteId='+getSiteId());"><dhv:label
            name="accounts.accounts_add.select">Select</dhv:label></a>] &nbsp;
          [<a
            href="javascript:document.forms['searchTicket'].searchcodeUserGroupId.value='-1';javascript:changeDivContent('changegroup',label('label.any','Any'));"><dhv:label
            name="button.clear">Clear</dhv:label></a>]
        </td>
      </tr>
    </table>
...
<dhv:evaluate if="<%= SiteList.size() > 2 %>">
  <tr>
    ...
    <td>
      ...
      <dhv:evaluate if="<%=User.getSiteId() > -1 %>">
        <input type="hidden" name="searchcodeSiteId"
...
<dhv:evaluate if="<%= SiteList.size() <= 2 %>">
  <input type="hidden" name="searchcodeSiteId" id="searchcodeSiteId"
         value="-1"/>
</dhv:evaluate>
<%--
<tr>
