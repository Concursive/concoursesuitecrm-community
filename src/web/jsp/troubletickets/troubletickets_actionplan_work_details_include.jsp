...
<jsp:useBean id="ticket_id" class="java.lang.String" scope="request"/>
<jsp:useBean id="ratingLookup" class="org.aspcfs.utils.web.LookupList"
             scope="request"/>
...
<script language="JavaScript" TYPE="text/javascript"
        SRC="javascript/popActionPlans.js"></script>
<script language="JavaScript" TYPE="text/javascript"
...
<%
  ...
  if (ticket_id != null && !"".equals(ticket_id)) {
    tid = Integer.parseInt(ticket_id);
  }
  if (tid > 0) {%>
<a href="TroubleTickets.do?command=Details&id=<%=tid%>"><dhv:label
    name="accounts.ticketsdetails">Ticket Details</dhv:label></a>
<%}%>
<br><br>
<%-- Plan Header --%>
<dhv:evaluate
    if="<%= (objectName != null && objectName.equals((String)constants.get(new Integer(ActionPlan.ACCOUNTS)))) %>">
  <table width="100%" border="0">
    <dhv:evaluate if="<%= hasText(actionPlanWork.getLinkItemName()) %>">
      <tr>
        <td><%= toHtml(actionPlanWork.getLinkItemName()) %>
        </td>
      </tr>
    </dhv:evaluate>
  </table>
...
<tr>
  <th>&nbsp;</th>
  <th>&nbsp;</th>
  <th style="border-bottom: 1px solid #9C9A9C;" width="40%" nowrap><dhv:label
      name="admin.step">Step</dhv:label></th>
  <th style="border-bottom: 1px solid #9C9A9C;" width="25%" nowrap><strong>Action</strong>
  </th>
  <th style="border-bottom: 1px solid #9C9A9C;" nowrap><strong><dhv:label
      name="reports.owner">Owner</dhv:label></strong></th>
</tr>
<%
...
<td rowspan="<%= thisPhaseWork.getItemWorkList().size() %>" nowrap
    ...
    >
  <strong><%= toHtml(thisPhaseWork.getPhaseName()) %>
  </strong>
  ...
  <dhv:evaluate if="<%= thisPhaseWork.getStartDate() != null %>">
    ...
    <dhv:evaluate if="<%= thisPhaseWork.getDaysInPhase() == 1 %>"><dhv:label
        name="actionPlan.day">day</dhv:label>)</dhv:evaluate>
    <dhv:evaluate
        if="<%= (thisPhaseWork.getDaysInPhase() == 0) || (thisPhaseWork.getDaysInPhase() > 1) %>"><dhv:label
        name="actionPlan.days">days</dhv:label>)</dhv:evaluate>
  </dhv:evaluate>
</td>
<% } %>
<%-- Action Plan Step --%>
<td nowrap valign="top"
    <dhv:evaluate
        if="<%= thisPhaseWork.getPhase().getRandom() && thisPhaseWork.getPhase().getId() == actionPlanWork.getCurrentPhaseId() %>">
    ...
    <dhv:evaluate
        if="<%= !thisPhaseWork.getPhase().getRandom() && !thisItemWork.isCurrent() %>">
      class="phaseStepNoneImage"
    </dhv:evaluate>
    <dhv:evaluate
        if="<%= !thisPhaseWork.getPhase().getRandom() && thisItemWork.isCurrent() %>">
...
<dhv:evaluate if="<%= thisPhaseWork.getPhase().getRandom() %>">
  <%-- Random Steps --%>
  <dhv:evaluate
      if="<%= thisItemWork.userHasPermission(User.getUserRecord().getId(), request) %>">
    <%-- User has permission to complete this step --%>
    <dhv:evaluate if="<%= thisPhaseWork.isCurrent() %>">
      <dhv:evaluate if="<%= !thisItemWork.hasStatus() %>">
        <a class="rollover"
           href="javascript:markComplete('<%= thisItemWork.getActionRequired() %>', '<%= thisItemWork.getActionId() %>','<%= actionPlanWork.getId() %>', '<%= thisItemWork.getId() %>', '-1');"><%= thisItemWork.getStatusGraphicTag(systemStatus) %>
        </a>
        <a class="phaseCurrentLink"
           href="javascript:markComplete('<%= thisItemWork.getActionRequired() %>', '<%= thisItemWork.getActionId() %>','<%= actionPlanWork.getId() %>', '<%= thisItemWork.getId() %>', '-1');"><%= toHtml(thisItemWork.getStepDescription()) %>
        </a>
      </dhv:evaluate>
      <dhv:evaluate if="<%= thisItemWork.hasStatus() %>">
        <a class="rollover"
           href="javascript:revertStatus('<%= actionPlanWork.getId() %>', '<%= thisItemWork.getId() %>', '-1');"><%= thisItemWork.getStatusGraphicTag(systemStatus) %>
        </a>
        <a class="phaseCurrentLink"
           href="javascript:revertStatus('<%= actionPlanWork.getId() %>', '<%= thisItemWork.getId() %>', '-1');"><%= toHtml(thisItemWork.getStepDescription()) %>
        </a>
      </dhv:evaluate>
    </dhv:evaluate>
    <dhv:evaluate if="<%= !thisPhaseWork.isCurrent() %>">
      ...
      <dhv:evaluate
          if="<%= (nextPhaseWork != null && nextPhaseWork.isCurrent() && nextPhaseWork.noStepComplete() && thisItemWork.hasStatus()) || (nextPhaseWork == null && thisItemWork.hasStatus()) %>">
        <a class="rollover"
           href="javascript:revertStatus('<%= actionPlanWork.getId() %>', '<%= thisItemWork.getId() %>', '-1');"><%= thisItemWork.getStatusGraphicTag(systemStatus) %>
        </a>
        <a class="rollover"
...
<a class="rollover" href="javascript:revertStatus('<%= actionPlanWork.getId() %>', '<%= thisItemWork.getId() %>', '<%= thisItemWork.getHasNext() ? thisItemWork.getNextStep().getId() : -1 %>');"><%= toHtml(thisItemWork.getStepDescription()) %></a--%>
        <a class="rollover"
           href="javascript:displayMsg();"><%= thisItemWork.getStatusGraphicTag(systemStatus) %>
        </a>
        <a class="rollover"
           href="javascript:displayMsg();"><%= toHtml(thisItemWork.getStepDescription()) %>
        </a>
        <%
            }
          } else
  ...
  <dhv:evaluate
    ...
    <dhv:evaluate if="<%= thisItemWork.isCurrent() %>">
      ...
      <dhv:evaluate if="<%= thisItemWork.getHasNext() %>">
        ...
        <dhv:evaluate
            if="<%= thisItemWork.getNextStep().userHasPermission(User.getUserRecord().getId(), request) %>">
          <%-- The Next step's owner is the current step's owner  --%>
          <a class="rollover"
             href="javascript:markComplete('<%= thisItemWork.getActionRequired() %>', '<%= thisItemWork.getActionId() %>','<%= actionPlanWork.getId() %>', '<%= thisItemWork.getId() %>', '<%= thisItemWork.getNextStep().getId() %>');"><%= thisItemWork.getStatusGraphicTag(systemStatus) %>
          </a>
          <a class="phaseCurrentLink"
             href="javascript:markComplete('<%= thisItemWork.getActionRequired() %>', '<%= thisItemWork.getActionId() %>','<%= actionPlanWork.getId() %>', '<%= thisItemWork.getId() %>', '<%= thisItemWork.getNextStep().getId() %>');"><%= toHtml(thisItemWork.getStepDescription()) %>
          </a>
        </dhv:evaluate>
        <dhv:evaluate
            if="<%= !thisItemWork.getNextStep().userHasPermission(User.getUserRecord().getId(), request) %>">
          ...
          <a class="rollover"
             href="javascript:checkPopURLReturn('<%= thisItemWork.getActionRequired() %>', '<%= thisItemWork.getActionId() %>','<%= actionPlanWork.getId() %>','<%= thisItemWork.getId() %>');"><%= thisItemWork.getStatusGraphicTag(systemStatus) %>
          </a>
          <a class="phaseCurrentLink"
             href="javascript:checkPopURLReturn('<%= thisItemWork.getActionRequired() %>', '<%= thisItemWork.getActionId() %>','<%= actionPlanWork.getId() %>','<%= thisItemWork.getId() %>');"><%= toHtml(thisItemWork.getStepDescription()) %>
          </a>
        </dhv:evaluate>
      </dhv:evaluate>
      <dhv:evaluate
          if="<%= !thisItemWork.getHasNext() && !thisItemWork.hasStatus() %>">
    ...
    <dhv:evaluate if="<%= !thisItemWork.isCurrent() %>">
      ...
      <dhv:evaluate if="<%= !thisItemWork.isPrevious() %>">
        ...
        <dhv:evaluate
            if="<%= thisItemWork.getAllowSkipToHere() && !thisItemWork.hasStatus() %>">
      ...
      <dhv:evaluate if="<%= thisItemWork.isPrevious() %>">
        ...
        <dhv:evaluate
          ...
          <a class="rollover"
             href="javascript:revertStatus('<%= actionPlanWork.getId() %>', '<%= thisItemWork.getId() %>', '<%= thisItemWork.getHasNext() ? thisItemWork.getNextStep().getId() : -1 %>');"><%= thisItemWork.getStatusGraphicTag(systemStatus) %>
          </a>
          <a class="rollover"
             href="javascript:revertStatus('<%= actionPlanWork.getId() %>', '<%= thisItemWork.getId() %>', '<%= thisItemWork.getHasNext() ? thisItemWork.getNextStep().getId() : -1 %>');"><%= toHtml(thisItemWork.getStepDescription()) %>
          </a>
        </dhv:evaluate>
        <dhv:evaluate
            if="<%= ! thisItemWork.userHasPermission(User.getUserRecord().getId(), request) %>">
...
<dhv:evaluate if="<%= thisItemWork.getStep().getQuickComplete() %>">
  <img src="images/approved.gif">
</dhv:evaluate>
</td>
<%-- Attachments --%>
<td valign="top" align="center" nowrap
    ...
    width="30%">
  <%@ include
      file="../actionplans/action_plan_work_details_attachments_include.jsp" %>
</td>
...
<td valign="top" nowrap
    ...
    >
  <dhv:evaluate
      if="<%= thisItemWork.getStep().getPermissionType() == ActionStep.USER_GROUP %>">
    <dhv:label name="usergroups.userGroup">User Group</dhv:label>:
    <dhv:evaluate
        if='<%= thisItemWork.getUserGroupName() != null && !"".equals(thisItemWork.getUserGroupName()) %>'>
    ...
    if="<%= thisItemWork.getStep().getPermissionType() == ActionStep.SPECIFIC_USER_GROUP %>">
  <dhv:label name="usergroups.userGroup">User Group</dhv:label>:
  <dhv:evaluate
      if='<%= thisItemWork.getUserGroupName() != null && !"".equals(thisItemWork.getUserGroupName()) %>'>
    <%= toHtml(thisItemWork.getUserGroupName()) %>
  ...
  <dhv:label name="usergroups.notSet">Not set</dhv:label>
    ...
    if="<%= thisItemWork.getStep().getPermissionType() == ActionStep.DEPARTMENT %>">
  <dhv:label
      name="project.department">Department</dhv:label>: <%= toHtml(thisItemWork.getDepartmentName()) %>
</dhv:evaluate><dhv:evaluate
    if="<%= thisItemWork.getStep().getPermissionType() == ActionStep.ROLE %>">
  <dhv:label
      name="accounts.accounts_add.Role">Role</dhv:label>: <%= toHtml(thisItemWork.getRoleName()) %>
</dhv:evaluate><dhv:evaluate
    if="<%= thisItemWork.getStep().getPermissionType() == ActionStep.WITHIN_USER_HIERARCHY %>">
  <dhv:label name="actionPlan.assignedUserHierarchy.withName"
             param='<%= "username="+ getUsername(pageContext, thisItemWork.getOwnerId(), false,true,"&nbsp;") %>'>Within
    the hierarchy
    of <%= getUsername(pageContext,thisItemWork.getOwnerId(),false,true,"&nbsp;") %>
  </dhv:label>
</dhv:evaluate><dhv:evaluate
    if="<%= thisItemWork.getStep().getPermissionType() == ActionStep.UP_USER_HIERARCHY %>">
  <dhv:label name="actionPlan.upTheUserHierarchy.withName"
             param='<%= "username="+ getUsername(pageContext, thisItemWork.getOwnerId(), false,true,"&nbsp;") %>'>Up
    the hierarchy
    of <%= getUsername(pageContext,thisItemWork.getOwnerId(),false,true,"&nbsp;") %>
  </dhv:label>
</dhv:evaluate><dhv:evaluate
    if="<%= thisItemWork.getStep().getPermissionType() == ActionStep.MANAGER %>">
  <dhv:username id="<%= actionPlanWork.getManagerId() %>"/>
</dhv:evaluate><dhv:evaluate
    if="<%= thisItemWork.getStep().getPermissionType() == ActionStep.ASSIGNED_USER_AND_MANAGER %>">
  <dhv:username id="<%= actionPlanWork.getManagerId() %>"/> <dhv:label name="">and</dhv:label>
