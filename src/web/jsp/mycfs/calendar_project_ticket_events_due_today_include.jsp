...
<%-- include tickets --%>
<dhv:evaluate if="<%= ticketEventList.getOpenProjectTickets().size() > 0 %>">
<table border="0" id="ticketsdetails<%= toFullDateString(thisDay.getDate()) %>" width="100%">
  <%-- title row --%>
  <tr>
    <td>&nbsp;</td>
    <td colspan="3" nowrap class="eventName">
      <img border="0" src="images/tree1.gif" align="absmiddle" title="Tickets" />
      <dhv:label name="tickets.projectTickets">Project Tickets</dhv:label>
      (<%= ticketEventList.getOpenProjectTickets().size() %>)
    </td>
  </tr>
  <%-- include ticket details --%>
  <%
    Iterator j = ticketEventList.getOpenProjectTickets().iterator();
    if(j.hasNext()){
  %>
    <tr>
      <th>
  ...
  <%  
    ...
    <tr>
     <td valign="top">
       ...
       <a href="javascript:displayProjectTicketMenu('select-arrow<%= menuCount %>','menuProjectTicket','<%=  thisTicket.getId() %>','<%=  thisTicket.getProjectId() %>');"
       onMouseOver="over(0, <%= menuCount %>)" onmouseout="out(0, <%= menuCount %>);hideMenu('menuProjectTicket');"><img
