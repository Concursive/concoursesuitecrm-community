...
<dhv:evaluate if="<%= (!Category.getAllowMultipleRecords()) %>">
  <a href="ExternalContacts.do?command=Fields&contactId=<%=ContactDetails.getId()%>&catId=<%= Category.getId() %>"><dhv:label name="accounts.accounts_fields.FolderRecordDetails">Folder Record Details</dhv:label></a> >
</dhv:evaluate>
...
<dhv:container name="contacts" selected="folders" object="ContactDetails" param="<%= "id=" + ContactDetails.getId() %>" appendToUrl="<%= addLinkParams(request, "popup|popupType|actionId") %>">
  ...
  <dhv:evaluate if="<%= !Category.isEmpty() %>">
    <br>
    <input type="submit" value="<dhv:label name="global.button.save">Save</dhv:label>" onClick="javascript:this.form.action='ExternalContacts.do?command=InsertFields&contactId=<%= ContactDetails.getId() %>&catId=<%= Category.getId() %>'">
