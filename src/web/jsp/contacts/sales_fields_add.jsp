<%-- 
  ...
  - 
  - Version: $Id: companydirectory_fields_add.jsp 18488 2007-01-15 20:12:32Z matt $
  - Description:
  --%>
...
<tr>
<td>
<a href="Sales.do"><dhv:label name="Co12ntacts" mainMenuItem="true">Leads</dhv:label></a> >
<a href="Sales.do?command=Details&id=<%=ContactDetails.getId()%>"><dhv:label name="accounts.accounts_contacts_add.Co1ntactDetails">Leads Details</dhv:label></a> >
...
<dhv:evaluate if="<%= (!Category.getAllowMultipleRecords()) %>">
  <a href="SalesFolders.do?command=Fields&contactId=<%=ContactDetails.getId()%>&catId=<%= Category.getId() %>"><dhv:label name="accounts.accounts_fields.FolderRecordDetails">Folder Record Details</dhv:label></a> >
</dhv:evaluate>
...
<dhv:container name="contacts" selected="folders" object="ContactDetails" param='<%= "id=" + ContactDetails.getId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
  ...
  <dhv:evaluate if="<%= !Category.isEmpty() %>">
    <br>
    <input type="submit" value="<dhv:label name="global.button.save">Save</dhv:label>" onClick="javascript:this.form.action='SalesFolders.do?command=InsertFields&contactId=<%= ContactDetails.getId() %>&catId=<%= Category.getId() %>'">
