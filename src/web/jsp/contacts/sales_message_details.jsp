<%-- 
  ...
  - 
  - Version: $Id: companydirectory_message_details.jsp 18876 2007-01-29 21:49:30Z matt $
  - Description: 
  --%>
...
<dhv:container name="leads" selected="messages" object="ContactDetails" param='<%= "id=" + ContactDetails.getId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId|from|listForm") %>'>
        ...
        <td>
          ...
          while (it.hasNext()) {
            ...
            <dhv:evaluate if="<%= thisAttachment.getFileExists() && hasPermissionDownload %>">
              <a href="DocumentSelector.do?command=Download&moduleId=<%=thisFile.getLinkModuleId() %>&linkItemId=<%= thisFile.getLinkItemId() %>&fid=<%= thisAttachment.getFileItemId() %>&ver=<%= thisAttachment.getVersion() %><%= addLinkParams(request, "popup|popupType|actionId|actionplan|from|listForm") %>">
