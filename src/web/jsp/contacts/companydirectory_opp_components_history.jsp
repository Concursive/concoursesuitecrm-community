...
<%@ include file="companydirectory_opp_components_history_menu.jsp" %>
  ...
  <dhv:hasAuthority owner="<%= String.valueOf(opportunityHeader.getManager()+(opportunityHeader.getManager() == thisComponentLog.getOwner()?"":","+thisComponentLog.getOwner())) %>">
    ...
    <tr class="row<%= rowid %>">
      ...
      <td width="100%" valign="top">
        <a href="ExternalContactsOppComponents.do?command=ComponentHistoryDetails&headerId=<%= opportunityHeader.getId() %>&contactId=<%= contactDetails.getId() %>&id=<%= thisComponentLog.getId() %>&return=details<%= addLinkParams(request, "viewSource|popup") %>">
