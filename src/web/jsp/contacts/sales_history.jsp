<%-- 
  ...
  - 
  - Version: $Id: sales_history.jsp 18488 2007-01-15 20:12:32Z yury.andreev $
  - Description:
  --%>
...
<%@ include file="sales_history_menu.jsp" %>
...
<script language="JavaScript" TYPE="text/javascript">
function reopen() {
  window.location.href='SalesHistory.do?command=List&contactId=<%= ContactDetails.getId() %>&popup=<%= isPopup(request) %><%= addLinkParams(request, "popup|popupType|actionId|from|listForm") %>';
}
</script>
<% String param2 = addLinkParams(request, "popup|popupType|actionId|from|listForm");%>
...
<td>
  ...
  <dhv:label name="sales.history">Lead History</dhv:label>
</td>
</tr>
</table>
<%-- End Trails --%>
</dhv:evaluate>
<dhv:container name="leads" selected="history" object="ContactDetails" param='<%= "id=" + ContactDetails.getId() %>' appendToUrl="<%= param2 %>">
...
<a href=" javascript:popURL('SalesHistory.do?command=AddNote&contactId=<%= ContactDetails.getId() %><%= addLinkParams(request, "popup|popupType|actionId|from|listForm") %>','Note','575','200','yes','yes');" ><dhv:label name="accounts.accountHistory.addANote">Add a Note</dhv:label></a><br /><br />
</dhv:permission></dhv:evaluate>
<span name="filterForm" id="filterForm" style="display:none">
<form name="history" action="SalesHistory.do?command=List&contactId=<%= ContactDetails.getId() %><%= param2 %>" method="post" onSubmit="return checkForm(this);">
...
<table cellpadding="4" cellspacing="0" class="empty"><tr><td>
  <table cellpadding="0" cellspacing="0" class="floatWrap"><tr><td valign="top" width="100%">
    <div><dhv:permission name="sales-leads-history-view">
    ...
    <div><dhv:permission name="sales-leads-calls-edit,sales-leads-calls-view">
    ...
    <div><dhv:permission name="myhomepage-inbox-view,myhomepage-inbox-edit,sales-leads-messages-view">
    ...
    <div><dhv:permission name="sales-leads-opportunities-edit,sales-leads-opportunities-view">
...
<dhv:pagedListStatus title='<%= showError(request, "actionError") %>' object="contactHistoryListInfo" externalJScript="flipFilterForm();" externalText="<dhv:label name=\"pagedListInfo.filters\">Filters</dhv:label>"/>
  <table cellpadding="4" cellspacing="0" width="100%" class="pagedList">
    <tr>
      ...
      <th nowrap>
        <strong><a href="SalesHistory.do?command=List&contactId=<%= ContactDetails.getId() %><%= addLinkParams(request, "popup|popupType|actionId|from|listForm") %>&column=type<%= isPopup(request)?"&popup=true":"" %>"><dhv:label name="reports.accounts.type">Type</dhv:label></a></strong>
      ...
      <th nowrap>
        <strong><a href="SalesHistory.do?command=List&contactId=<%= ContactDetails.getId() %><%= addLinkParams(request, "popup|popupType|actionId|from|listForm") %>&column=entered<%= isPopup(request)?"&popup=true":"" %>"><dhv:label name="accounts.accounts_calls_list.Entered">Entered</dhv:label></a></strong>
        <%= contactHistoryListInfo.getSortIcon("entered") %>
      </th>
      <th nowrap>
        <strong><a href="SalesHistory.do?command=List&contactId=<%= ContactDetails.getId() %><%= addLinkParams(request, "popup|popupType|actionId|from|listForm") %>&column=modified<%= isPopup(request)?"&popup=true":"" %>"><dhv:label name="accounts.accounts_contacts_calls_details.Modified">Modified</dhv:label></a></strong>
