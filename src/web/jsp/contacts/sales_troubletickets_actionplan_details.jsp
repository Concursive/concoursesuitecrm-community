...
<script language="JavaScript" TYPE="text/javascript">
    ...
    if (required == 'true') {
      if ((actionId == '<%= ActionStep.UPDATE_RATING %>') && (document.actionPlan.rating.options[document.actionPlan.rating.selectedIndex].value == -1)) {
        ...
        <dhv:evaluate if="<%= (objectName != null && objectName.equals((String)constants.get(new Integer(ActionPlan.ACCOUNTS)))) %>">
          popURLReturn('SalesActionPlans.do?command=ModifyStatus&contactId=<%= ContactDetails.getId() %>&planId=' + planId + '&itemId=' + itemId,'AccountActionPlans.do?command=Details&orgId=<%= ContactDetails.getOrgId() %>&actionPlanId=' + planId,'Action_Plan',550,200);
        </dhv:evaluate>
      }
    } else {
      <dhv:evaluate if="<%= objectName != null && objectName.equals((String)constants.get(new Integer(ActionPlan.TICKETS))) %>">
      ...
      <dhv:evaluate if="<%= (objectName != null && objectName.equals((String)constants.get(new Integer(ActionPlan.ACCOUNTS)))) %>">
        popURLReturn('SalesActionPlans.do?command=ModifyStatus&contactId=<%= ContactDetails.getId() %>&planId=' + planId + '&itemId=' + itemId,'SalesActionPlans.do?command=Details&contactId=<%= ContactDetails.getId() %>&actionPlanId=' + planId,'Action_Plan',550,200);
      </dhv:evaluate>
    }
  }
  ...
  function reviewNotes() {
    ...
    <dhv:evaluate if="<%= (objectName != null && objectName.equals((String)constants.get(new Integer(ActionPlan.ACCOUNTS)))) %>">
      popURL('SalesActionPlans.do?command=ViewNotes&contactId=<%= ContactDetails.getId() %>&planWorkId=<%= actionPlanWork.getId() %>','Action_Plan',700,425,'yes','yes');
    </dhv:evaluate>
  }
  ...
  function restartPlan() {
    ...
    <dhv:evaluate if="<%= (objectName != null && objectName.equals((String)constants.get(new Integer(ActionPlan.ACCOUNTS)))) %>">
      confirmForward('SalesActionPlans.do?command=Restart&contactId=<%= ContactDetails.getId() %>&actionPlanId=<%= actionPlanWork.getId() %><%= addLinkParams(request, "popup|popupType|actionId") %>');
    </dhv:evaluate>
  }
  ...
  function deletePlan() {
    ...
    <dhv:evaluate if="<%= (objectName != null && objectName.equals((String)constants.get(new Integer(ActionPlan.ACCOUNTS)))) %>">
      confirmDelete('SalesActionPlans.do?command=Delete&contactId=<%= ContactDetails.getId() %>&actionPlanId=<%= actionPlanWork.getId() %><%= addLinkParams(request, "popup|popupType|actionId") %>');
    </dhv:evaluate>
  }
                        ...
                        <dhv:evaluate if="<%= !thisPhaseWork.isCurrent() %>">
                          ...
                          <dhv:evaluate if="<%= nextPhaseWork != null && (!nextPhaseWork.isCurrent() || !nextPhaseWork.noStepComplete() || !thisItemWork.hasStatus()) || (nextPhaseWork == null && !thisItemWork.hasStatus()) %>">
                            ...
                            if (!thisItemWork.isComplete()) {%>
                              <a class="rollover" href="javascript:markComplete('<%= thisItemWork.getActionRequired() %>', '<%= thisItemWork.getActionId() %>','<%= actionPlanWork.getId() %>', '<%= thisItemWork.getId() %>', '-1');"><%= thisItemWork.getStatusGraphicTag(systemStatus) %></a>
                              <a class="rollover" href="javascript:markComplete('<%= thisItemWork.getActionRequired() %>', '<%= thisItemWork.getActionId() %>','<%= actionPlanWork.getId() %>', '<%= thisItemWork.getId() %>', '-1');"><%= toHtml(thisItemWork.getStepDescription()) %></a>
                            <%} else {%>
