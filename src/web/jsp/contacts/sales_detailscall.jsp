<%-- 
  ...
  - 
  - Version: $Id: companydirectory_detailscall.jsp 18488 2007-01-15 20:12:32Z matt $
  - Description: 
  --%>
...
<form name="addCall" action="SalesCalls.do?command=Modify&id=<%= CallDetails.getId() %>&contactId=<%= ContactDetails.getId() %>" method="post">
<dhv:evaluate if="<%= !isPopup(request) %>">
<%-- Trails --%>
<table class="trails" cellspacing="0">
<tr>
<td>
<a href="Sales.do"><dhv:label name="accounts.tacts">Leads</dhv:label></a> > 
...
<dhv:container name="leads" selected="activities" object="ContactDetails" param='<%= "id=" + ContactDetails.getId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
  <dhv:evaluate if="<%= ContactDetails.getEnabled() && !ContactDetails.isTrashed() && !CallDetails.isTrashed() %>">
    <dhv:evaluate if='<%= "pending".equals(request.getParameter("view")) %>'>
      <dhv:sharing primaryBean="CallDetails" secondaryBeans="ContactDetails" action="edit"><input type="button" value="<dhv:label name="global.button.complete">Complete</dhv:label>" onClick="javascript:window.location.href='SalesCalls.do?command=Complete&contactId=<%= ContactDetails.getId() %>&id=<%= CallDetails.getId() %><%= addLinkParams(request, "view|popup|popupType|actionId") %>'"></dhv:sharing>
    </dhv:evaluate>
    ...
    <%}%>
    <dhv:evaluate if='<%= "pending".equals(request.getParameter("view")) %>'>
      <dhv:sharing primaryBean="CallDetails" secondaryBeans="ContactDetails" action="delete"><input type="button" value="<dhv:label name="global.button.CancelPendingActivity">Cancel Pending Activity</dhv:label>" onClick="javascript:window.location.href='SalesCalls.do?command=Cancel&contactId=<%= CallDetails.getContactId() %>&id=<%= CallDetails.getId() %>&action=cancel<%= addLinkParams(request, "popup|popupType|actionId|view") %>';"></dhv:sharing>
    </dhv:evaluate>
    <dhv:evaluate if="<%= !isPopup(request) %>">
      <dhv:permission name="sales-leads-calls-view"><input type="button" name="action" value="<dhv:label name="accounts.accounts_calls_list_menu.Forward"></dhv:label>" onClick="javascript:window.location.href='SalesCalls.do?command=ForwardCall&contactId=<%= ContactDetails.getId() %>&id=<%= CallDetails.getId() %>&forwardType=<%= Constants.TASKS %><%= addLinkParams(request, "popup|popupType|actionId|view") %>'"></dhv:permission>
    </dhv:evaluate>
  </dhv:evaluate>
  <dhv:permission name="sales-leads-calls-edit,sales-leads-calls-delete,sales-leads-calls-view"><br>&nbsp;</dhv:permission>
  <% if("pending".equals(request.getParameter("view"))){ %>
  ...
  <dhv:evaluate if="<%= ContactDetails.getEnabled() && !ContactDetails.isTrashed() && !CallDetails.isTrashed() %>">
   ...
   <% if("pending".equals(request.getParameter("view"))){ %>
    ...
    <%}%>
    <dhv:evaluate if='<%= "pending".equals(request.getParameter("view")) %>'>
      <dhv:sharing primaryBean="CallDetails" secondaryBeans="ContactDetails" action="delete"><input type="button" value="<dhv:label name="global.button.CancelPendingActivity">Cancel Pending Activity</dhv:label>" onClick="javascript:window.location.href='SalesCalls.do?command=Cancel&contactId=<%= ContactDetails.getId() %>&id=<%= CallDetails.getId() %>&action=cancel<%= addLinkParams(request, "popup|popupType|actionId|view") %>';"></dhv:sharing></dhv:evaluate>
      <dhv:evaluate if="<%= !isPopup(request) %>">
      <dhv:permission name="sales-leads-calls-view"><input type="button" name="action" value="<dhv:label name="accounts.accounts_calls_list_menu.Forward">Forward</dhv:label>" onClick="javascript:window.location.href='SalesCalls.do?command=ForwardCall&contactId=<%= ContactDetails.getId() %>&forwardType=<%= Constants.TASKS %>&id=<%= CallDetails.getId() %><%= addLinkParams(request, "popup|popupType|actionId|view") %>'"></dhv:permission>
    </dhv:evaluate>
  </dhv:evaluate>
