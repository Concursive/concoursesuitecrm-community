...
<table cellpadding="0" cellspacing="0" width="100%" border="1" rules="cols">
  <tr class="section">
    <td valign="top" width="100%">
      ...
      <a href="ExternalContactsDocuments.do?command=SaveMove&contactId=<%= ContactDetails.getId() %>&fid=<%= FileItem.getId() %>&popup=true&folderId=0&return=ExternalContactsDocuments&param=<%= ContactDetails.getId() %>&param2=<%= FileItem.getFolderId() %>"><dhv:label name="accounts.accounts_documents_file_move.Home">Home</dhv:label></a>
...
%>
  <tr class="row<%= rowid %>">
    ...
    <% for(int j=1;j<thisFolder.getLevel();j++){ %>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<%}%>
      ...
      <a href="ExternalContactsDocuments.do?command=SaveMove&contactId=<%= ContactDetails.getId() %>&fid=<%= FileItem.getId() %>&popup=true&folderId=<%= thisFolder.getId() %>&return=ExternalContactsDocuments&param=<%= ContactDetails.getId() %>&param2=<%= FileItem.getFolderId() %>"><%= toHtml(thisFolder.getSubject()) %></a>
