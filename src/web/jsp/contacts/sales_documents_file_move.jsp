<%-- 
  ...
  - 
  - Version: $Id: contacts_documents_file_move.jsp 17754 2006-12-11 vadim.vishnevsky@corratech.com $
  - Description: 
  --%>
...
<table cellpadding="0" cellspacing="0" width="100%" border="1" rules="cols">
  <tr class="section">
    <td valign="top" width="100%">
      ...
      <a href="SalesDocuments.do?command=SaveMove&contactId=<%= ContactDetails.getId() %>&fid=<%= FileItem.getId() %>&popup=true&folderId=0&return=SalesDocuments&param=<%= ContactDetails.getId() %>&param2=<%= FileItem.getFolderId() %>"><dhv:label name="accounts.accounts_documents_file_move.Home">Home</dhv:label></a>
...
%>
  <tr class="row<%= rowid %>">
    ...
    <% for(int j=1;j<thisFolder.getLevel();j++){ %>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<%}%>
      ...
      <a href="SalesDocuments.do?command=SaveMove&contactId=<%= ContactDetails.getId() %>&fid=<%= FileItem.getId() %>&popup=true&folderId=<%= thisFolder.getId() %>&return=SalesDocuments&param=<%= ContactDetails.getId() %>&param2=<%= FileItem.getFolderId() %><%= addLinkParams(request, "popup|popupType|actionId|from|listForm") %>"><%= toHtml(thisFolder.getSubject()) %></a>
