<%-- 
  ...
  - 
  - Version: $Id: companydirectory_detailsopp.jsp 18488 2007-01-15 20:12:32Z matt $
  - Description:
  --%>
...
<%@ include file="sales_detailsopp_menu.jsp" %>
  ...
  function reopenOnDelete() {
    try {
      if ('<%= isPopup(request) %>' != 'true') {
        ...
        scrollReload('SalesOpportunitiess.do?command=ViewOpps&contactId=<%= ContactDetails.getId() %>&popup=true');
        var oppId = -1;
        try {
...
<dhv:container name="leads" selected="opportunities" object="ContactDetails" param='<%= "id=" + ContactDetails.getId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId|from|listForm") %>'>
    ...
    <dhv:evaluate if="<%= ContactDetails.getEnabled() && !ContactDetails.isTrashed() && !opportunityHeader.isTrashed() %>">
      ...
      <dhv:permission name="sales-leads-opportunities-add,accounts-accounts-contacts-opportunities-add"  all="true">
          <dhv:evaluate if="<%=allowMultiple || (!allowMultiple && (componentList.size() == 0))%>" >
            <br /><a href="SalesOpportunitiesComponents.do?command=Prepare&headerId=<%= opportunityHeader.getId() %>&contactId=<%= opportunityHeader.getContactLink() %><%= addLinkParams(request, "popup|popupType|actionId|from|listForm") %>"><dhv:label name="accounts.accounts_contacts_opps_details.AddAComponent">Add a Component</dhv:label></a><br />
          </dhv:evaluate>
        </dhv:permission>
      <% }else{ %>
        <dhv:permission name="sales-leads-opportunities-add">
          <br />
          <a href="SalesOpportunitiesComponents.do?command=Prepare&headerId=<%= opportunityHeader.getId() %>&contactId=<%= opportunityHeader.getContactLink() %><%= addLinkParams(request, "popup|popupType|actionId|from|listForm") %>"><dhv:label name="accounts.accounts_contacts_opps_details.AddAComponent">Add a Component</dhv:label></a><br />
        </dhv:permission>
      <%}%>
    </dhv:evaluate>
  </dhv:evaluate>
  ...
  <table cellpadding="4" cellspacing="0" border="0" width="100%" class="pagedList">
    <tr>
      <th width="8">&nbsp;</th>
      <th nowrap>
        <a href="SalesOpportunities.do?command=DetailsOpp&headerId=<%= opportunityHeader.getId() %>&contactId=<%= ContactDetails.getId() %>&column=oc.description<%= addLinkParams(request, "popup|popupType|actionId|from|listForm") %>"><dhv:label name="accounts.accounts_contacts_opps_details.Component">Component</dhv:label></a>
        <%= ComponentListInfo.getSortIcon("oc.description") %>
      </th>
      <th nowrap>
        <a href="SalesOpportunities.do?command=DetailsOpp&headerId=<%= opportunityHeader.getId() %>&contactId=<%= ContactDetails.getId() %>&column=oc.closed<%= addLinkParams(request, "popup|popupType|actionId|from|listForm") %>"><dhv:label name="accounts.accountasset_include.Status">Status</dhv:label></a>
        <%= ComponentListInfo.getSortIcon("oc.closed") %>
      </th>
      <th nowrap>
        <a href="SalesOpportunities.do?command=DetailsOpp&headerId=<%= opportunityHeader.getId() %>&contactId=<%= ContactDetails.getId() %>&column=oc.guessvalue<%= addLinkParams(request, "popup|popupType|actionId|from|listForm") %>"><dhv:label name="accounts.accounts_contacts_opps_details.GuessAmount">Guess Amount</dhv:label></a>
        <%= ComponentListInfo.getSortIcon("oc.guessvalue") %>
      </th>
      <th nowrap>
        <a href="SalesOpportunities.do?command=DetailsOpp&headerId=<%= opportunityHeader.getId() %>&contactId=<%= ContactDetails.getId() %>&column=oc.closedate<%= addLinkParams(request, "popup|popupType|actionId|from|listForm") %>"><dhv:label name="accounts.accounts_contacts_opps_details.CloseDate">Close Date</dhv:label></a>
        <%= ComponentListInfo.getSortIcon("oc.closedate") %>
      </th>
      <th nowrap>
        <a href="SalesOpportunities.do?command=DetailsOpp&headerId=<%= opportunityHeader.getId() %>&contactId=<%= ContactDetails.getId() %>&column=stagename<%= addLinkParams(request, "popup|popupType|actionId|from|listForm") %>"><dhv:label name="accounts.accounts_contacts_oppcomponent_details.CurrentStage">Current Stage</dhv:label></a>
  ...
  %>
    ...
    <tr class="containerBody">
      ...
      <td width="100%" valign="top" class="row<%= rowid %>">
        <a href="SalesOpportunitiesComponents.do?command=DetailsComponent&contactId=<%= ContactDetails.getId() %>&id=<%= oppComponent.getId() %><%= addLinkParams(request, "popup|popupType|actionId|from|listForm") %>">
  ...
  <dhv:hasAuthority owner="<%= opportunityHeader.getManager() %>">
    <dhv:evaluate if="<%= ContactDetails.getEnabled() && !ContactDetails.isTrashed() && !opportunityHeader.isTrashed() %>">
      <dhv:evaluate if="<%= ContactDetails.getEnabled() && !ContactDetails.isTrashed() && !opportunityHeader.isTrashed() %>">
        <dhv:sharing primaryBean="opportunityHeader" secondaryBeans="ContactDetails" action="edit"><input type="button" value="<dhv:label name="global.button.RenameOpportunity">Rename Opportunity</dhv:label>" onClick="javascript:window.location.href='SalesOpportunities.do?command=ModifyOpp&headerId=<%= opportunityHeader.getId() %>&contactId=<%= opportunityHeader.getContactLink() %><%= addLinkParams(request, "popup|popupType|actionId|from|listForm") %>';"></dhv:sharing>
        <dhv:sharing primaryBean="opportunityHeader" secondaryBeans="ContactDetails" action="delete"><input type="button" value="<dhv:label name="global.button.DeleteOpportunity">Delete Opportunity</dhv:label>" onClick="javascript:popURLReturn('SalesOpportunities.do?command=ConfirmDelete&contactId=<%= ContactDetails.getId() %>&headerId=<%= opportunityHeader.getId() %>&popup=true<%= isPopup(request)?"&sourcePopup=true":"" %><%= addLinkParams(request, "popupType|actionId|from|listForm") %>','ExternalContactsOpps.do?command=ViewOpps&contactId=<%= ContactDetails.getId() %>', 'Delete_opp','320','200','yes','no')"></dhv:sharing>
      </dhv:evaluate>
    </dhv:evaluate>
  </dhv:hasAuthority>
</dhv:container>
