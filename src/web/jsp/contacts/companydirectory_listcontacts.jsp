...
%>      
      <tr>
        ...
        <td width=8 valign=center nowrap class="row<%= rowid %>">
          <dhv:permission name="contacts-external_contacts-edit"><a href="/ExternalContacts.do?command=ContactDetails&id=<%= thisContact.getId()%>&action=modify&return=list">Edit</a></dhv:permission><dhv:permission name="contacts-external_contacts-edit,contacts-external_contacts-delete" all="true">|</dhv:permission><dhv:permission name="contacts-external_contacts-delete"><dhv:ssl><a href="javascript:popURLReturn('https://<%= request.getServerName() %>/ExternalContacts.do?command=ConfirmDelete&id=<%=thisContact.getId()%>','ExternalContacts.do?command=ListContacts', 'Delete_contact','330','200','yes','no');">Del</a></dhv:ssl><dhv:ssl none="true"><a href="javascript:popURLReturn('http://<%= request.getServerName() %>/ExternalContacts.do?command=ConfirmDelete&id=<%=thisContact.getId()%>','ExternalContacts.do?command=ListContacts', 'Delete_contact','330','200','yes','no');">Del</a></dhv:ssl></dhv:permission>
        </td>
	</dhv:permission>
