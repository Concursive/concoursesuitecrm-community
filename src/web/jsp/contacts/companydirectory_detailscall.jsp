...
<dhv:permission name="myhomepage-inbox-view"><input type="button" name="action" value="Forward" onClick="javascript:window.location.href='ExternalContactsCallsForward.do?command=ForwardMessage&popup=true&forwardType=8&id=<%=CallDetails.getId()%>&return='+escape('ExternalContactsCalls.do?command=Details&id=<%=CallDetails.getId()%>&contactId=<%=ContactDetails.getId()%>') + '&sendUrl=' + escape('ExternalContactsCalls.do?command=SendMessage&contactId=<%= ContactDetails.getId() %>');"></dhv:permission>
