...
<jsp:useBean id="SalesImportResultsInfo" class="org.aspcfs.utils.web.PagedListInfo" scope="session"/>
<%@ include file="../initPage.jsp" %>
<%-- Initialize the drop-down menus --%>
<%@ include file="../initPopupMenu.jsp" %>
<%@ include file="sales_importresults_menu.jsp" %>
...
<center><dhv:pagedListAlphabeticalLinks object="SalesImportResultsInfo"/></center></dhv:include>
<table width="100%" border="0">
  <tr>
    <td>
      <dhv:pagedListStatus title='<%= showError(request, "actionError") %>' object="SalesImportResultsInfo"/>
    </td>
  </tr>
</table>
<table cellpadding="4" cellspacing="0" border="0" width="100%" class="pagedList">
  <tr>
    ...
    <th nowrap>
      <strong><a href="SalesImports.do?command=ViewResults&importId=<%= ImportDetails.getId() %>&column=c.namelast"><dhv:label name="contacts.name">Name</dhv:label></a></strong>
      <%= SalesImportResultsInfo.getSortIcon("c.namelast") %>
    </th>
    <th nowrap>
      <strong><a href="SalesImports.do?command=ViewResults&importId=<%= ImportDetails.getId() %>&column=c.org_name"><dhv:label name="accounts.accounts_contacts_detailsimport.Company">Company</dhv:label></a></strong>
      <%= SalesImportResultsInfo.getSortIcon("c.org_name") %>
    </th>
    <th>
      <strong><dhv:label name="account.phones.colon">Phone(s):</dhv:label></strong>
    </th>
    <dhv:evaluate if='<%= !"my".equals(SalesImportResultsInfo.getListView()) && !"".equals(SalesImportResultsInfo.getListView()) %>'>
      <th>
...
%>    
      <tr>
        ...
        <%
          ...
          <% }else{ %>
              <dhv:permission name="sales-import-view" all="true">
                <% hasDeletePermission = 1; %>
              </dhv:permission>
          <% } %>
        <% } %>
        ...
        <td class="row<%= rowid %>" <%= "".equals(toString(thisContact.getNameLastFirst())) ? "width=\"10\"" : ""  %> nowrap>
          ...
          <a href="SalesImports.do?command=ContactDetails&contactId=<%= thisContact.getId() %>"><%= toHtml(thisContact.getNameLastFirst()) %></a>
          <% }else{ %>
            &nbsp;
          <%}%>
        </td>
...
<%
  ...
  <tr>
    <td class="containerBody" colspan="5">
      <dhv:label name="sales.noLeadsFound">No leads found.</dhv:label>
    </td>
  </tr>
<%}%>
</table>
<br>
<dhv:pagedListControl object="SalesImportResultsInfo" tdClass="row1"/>
