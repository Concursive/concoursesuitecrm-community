...
<%@ include file="companydirectory_employee_fields_list_menu.jsp" %>
...
<dhv:container name="employees" selected="folders" object="ContactDetails" param='<%= "id=" + ContactDetails.getId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
  ...
  <% if (CategoryList.size() > 0) { %>
    &nbsp;<br>
    <dhv:evaluate if="<%= (!Category.getReadOnly()) %>"><dhv:permission name="contacts-internal_contacts-folders-add"><a href="CompanyDirectory.do?command=AddFolderRecord&empid=<%= ContactDetails.getId() %>&catId=<%= Category.getId() %><%= addLinkParams(request, "popup|popupType|actionId") %>"><dhv:label name="accounts.accounts_fields_list.AddRecordToFolder">Add a record to this folder</dhv:label></a><br>&nbsp;<br></dhv:permission></dhv:evaluate>
...
%>    
      <tr class="containerBody">
        ...
        <td align="left" width="100%" nowrap class="row<%= rowid %>">
          <a href="CompanyDirectory.do?command=Fields&empid=<%= ContactDetails.getId() %>&catId=<%= Category.getId() %>&recId=<%= thisRecord.getId() %><%= addLinkParams(request, "popup|popupType|actionId") %>"><%= thisRecord.getFieldData() != null ? thisRecord.getFieldData().getValueHtml(false) : "&nbsp;" %></a>
        </td>
