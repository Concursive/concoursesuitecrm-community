...
<form name="pricingForm" action="ProductCatalogPricings.do?command=UpdatePricing&auto-populate=true" onSubmit="return doCheck(this);" method="post">
<%-- Trails --%>
...
<table border="0" cellpadding="1" cellspacing="0" width="100%">
  <tr class="abovetab">
    <td>
      ...
      <a href="ProductCatalogPricings.do?command=PricingDetails&pricingId=<%= Pricing.getId() %>&productId=<%= productCatalog.getId() %>&moduleId=<%= permissionCategory.getId() %>&categoryId=<%= productCategory.getId() %>"><dhv:label name="product.priceDetails">Price Details</dhv:label></a> >
...
<table cellpadding="4" cellspacing="0" border="0" width="100%">
  <tr>
    <td>
      <strong><dhv:label name="product.priceNumber.symbol">Price #</dhv:label><%= Pricing.getId() %></strong><br><br>
		 	<input type="submit" value="<dhv:label name="button.update">Update</dhv:label>" name="Save" onClick="this.form.dosubmit.value='true'"/>
			<input type="submit" value="<dhv:label name="button.cancel">Cancel</dhv:label>" onClick="javascript:this.form.action='ProductCatalogPricings.do?command=PricingDetails&productId=<%= productCatalog.getId() %>&moduleId=<%= permissionCategory.getId() %>&categoryId=<%= productCategory.getId() %>'"/><br>
      ...
      <input type="hidden" name="pricingId" value="<%= Pricing.getId() %>" />
      <input type="hidden" name="id" value="<%= Pricing.getId() %>" />
			<input type="submit" value="<dhv:label name="button.update">Update</dhv:label>" name="Save" onClick="this.form.dosubmit.value='true'"/>
			<input type="submit" value="<dhv:label name="button.cancel">Cancel</dhv:label>" onClick="javascript:this.form.action='ProductCatalogPricings.do?command=PricingDetails&productId=<%= productCatalog.getId() %>&moduleId=<%= permissionCategory.getId() %>&categoryId=<%= productCategory.getId() %>'"/>
