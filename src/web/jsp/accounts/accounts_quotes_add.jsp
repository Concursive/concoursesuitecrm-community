...
<form method="post" name="addQuote" action="AccountQuotes.do?command=AddQuote&auto-populate=true<%= addLinkParams(request, "popup|popupType|actionId") %>" onSubmit="return checkForm(this);">
...
<dhv:container name="accounts" selected="quotes" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
  ...
  <input type="button" value="<dhv:label name="button.cancel">Cancel</dhv:label>" onClick="javascript:window.location.href='AccountQuotes.do?command=View&orgId=<%= OrgDetails.getId() %><%= addLinkParams(request, "popup|popupType|actionId") %>';"/>
  <br />
...
%>
  ...
  <input type="button" value="<dhv:label name="global.button.cancel">Cancel</dhv:label>" onClick="javascript:window.location.href='AccountQuotes.do?command=View&orgId=<%= OrgDetails.getId() %><%= addLinkParams(request, "popup|popupType|actionId") %>';" />
</dhv:container>
</form>
