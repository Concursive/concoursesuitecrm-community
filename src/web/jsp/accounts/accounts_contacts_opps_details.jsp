      ...
      <tr class="row<%= rowid %>">
        <td width="8" valign="top" nowrap>
          ...
          <dhv:evaluate if="<%= ContactDetails.getEnabled() && !ContactDetails.isTrashed() && !OpportunityHeader.isTrashed() %>">
            <a href="javascript:displayMenu('select<%= i %>','menuOpp','<%= OpportunityHeader.getId() %>', '<%= oppComponent.getId() %>', '<%= OpportunityHeader.getContactLink() %>','<%= OpportunityHeader.isTrashed() %>','<%= hasPermission %>');" onMouseOver="over(0, <%= i %>)" onmouseout="out(0, <%= i %>); hideMenu('menuOpp');"><img src="images/select.gif" name="select<%= i %>" id="select<%= i %>" align="absmiddle" border="0"></a>
          </dhv:evaluate>
