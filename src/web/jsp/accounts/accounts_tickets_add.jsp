...
<jsp:useBean id="ContactList"
             ...
             scope="request"/>
<jsp:useBean id="SubmitterContactList"
             class="org.aspcfs.modules.contacts.base.ContactList"
             scope="request"/>
...
<dhv:include name="ticket.contact" none="true">
  var orgWidget = document.forms['addticket'].elements['subId'];
  var subValue = document.forms['addticket'].subId.value;
...
<dhv:evaluate if="<%= (!User.getUserRecord().isPortalUser()) %>" >
  ...
  var params = "&subId=" + escape(subValue);
  params = params + "&orgId=" + escape(orgValue);
<dhv:evaluate if="<%= (!User.getUserRecord().isPortalUser()) %>" >
  ...
  var url = "AccountTickets.do?command=OrganizationJSList" + params;
  window.frames['server_commands'].location.href = url;
</dhv:include>
}
...
<dhv:include name="ticket.contact" none="true">
  var sel = document.forms['addticket'].elements['subId'];
  var value = sel.value;
  var url = "AccountTickets.do?command=OrganizationJSList&subId=" + escape(value);
  window.frames['server_commands'].location.href = url;
</dhv:include>
}
...
<dhv:include name="ticket.contact" none="true">
  if (form.contactId.value == "-1") {
    message += label("check.ticket.contact.entered", "- Check that a Submitter contact is selected\r\n");
    formTest = false;
  }
</dhv:include>

  if (form.subId.value == "-1") {
    message += label("check.ticket.account.entered", "- Check that a Organization is selected\r\n");
    formTest = false;
  }
  if (form.submitterContactId.value == "-1") {
...
function insertOption(text, value, optionListId) {
  var obj = document.forms['addticket'].elements[optionListId];
...
<dhv:permission name="accounts-accounts-contacts-add" none="true">
  ...
  var orgId = document.forms['addticket'].subId.value;
  if (orgId == -1) {
    alert(label("select.account.first", 'You have to select an Account first'));
    return;
  }
  if (acctPermission) {
    popURL('Contacts.do?command=Prepare&popup=true&orgId=' + orgId + '&popup=true&hiddensource=addticket&fieldName=submitterContactId', 'New_Contact', '600', '550', 'yes', 'yes');
  } else {
...
function selectServiceContract() {
  ...
  if (orgId == -1) {
    ...
    popServiceContractListSingle('contractId', 'addServiceContract', 'filters=all|my|disabled', orgId);
  }
  return;
}

function selectServiceContract2() {
  var orgId = document.forms['addticket'].subId.value;
  var subId = document.forms['addticket'].orgId.value;
  if (orgId == -1) {
    alert(label("select.account.first", 'You have to select an Account first'));
    return;
  } else {
    popServiceContractListSingle2('contractId', 'addServiceContract', 'filters=all|my|disabled', orgId, subId);
  }
  return;
}
...
function selectAsset() {
  ...
  if (orgId == -1) {
    ...
    popAssetListSingle('assetId', 'addAsset', 'filters=allassets|undercontract', 'contractId', 'addServiceContract', orgId);
  }
  return;
}

function selectAsset2() {
  var orgId = document.forms['addticket'].subId.value;
  var subId = document.forms['addticket'].orgId.value;
  if (orgId == -1) {
    alert(label("select.account.first", 'You have to select an Account first'));
    ...
    popAssetListSingle2('assetId', 'addAsset', 'filters=allassets|undercontract', 'contractId', 'addServiceContract', orgId, '', subId);
  }
  return;
}
...
%>
<dhv:permission name="accounts-accounts-tickets-reseller-restriction-view">
  <%
    isReseller = true;
  %>
</dhv:permission>
...
<form name="addticket"
      action="AccountTickets.do?command=InsertTicket&auto-populate=true<%= addLinkParams(request, "popup|popupType|actionId") %>"
      method="post">
...
<input type="submit" value="<dhv:label name="button.insert">Insert</dhv:label>"
       name="Save" onClick="return checkForm(this.form);"/>
<input type="submit"
...
<tr>
  <th colspan="2">
    <strong><dhv:label name="accounts.tickets.add">Add a new
      Ticket</dhv:label></strong>
  </th>
</tr>
<dhv:evaluate if="<%= (User.getRoleType() == 0) %>">
...
<tr class="containerBody">
  <td class="formLabel">
    <dhv:label name="ticket.submiteridLink">Submitter</dhv:label>
  </td>
  <td>
    ...
    <input type="hidden" name="orgSiteId" id="orgSiteId"
           value="<%=  OrgDetails.getSiteId() %>"/>
  </td>
</tr>
<tr class="containerBody">
  <td class="formLabel">
    <dhv:label name="accounts.accountasset_include.submiterContact">Submitter
      Contact</dhv:label>
  </td>
  <td>
    ...
    <%} else {%>
    <%= ContactList.getHtmlSelect("contactId", User.getContact().getId()) %>
    <%}%>
    <font color="red">*</font><%= showAttribute(request, "contactIdError") %>
    <dhv:evaluate if="<%= (User.getRoleType() == 0) %>">
      [<a
      href="javascript:popURL('Contacts.do?command=Prepare&popup=true&orgId=<%= OrgDetails.getOrgId() %>
      &popup=true&hiddensource=addticket',
      'New_Contact','600','550','yes','yes');"><dhv:label
...
<tr>
  <td class="formLabel">
    <dhv:label
        name="accounts.accounts_add.Organization">Organization</dhv:label>
  </td>
  <td>
    <table cellspacing="0" cellpadding="0" border="0" class="empty">
      <tr>
        ...
        <td>
          ...
          [<a
            href="javascript:popAccountsListSingle2('subId','changesubmitter', 'showMyCompany=true&filters=all|my|disabled&orgid=<%=OrgDetails.getId() %>');"><dhv:label
...
<dhv:include name="ticket.contact" none="true">
  <tr>
    ...
    <td valign="center">
      <% if (SubmiterOrgDetails == null || SubmiterOrgDetails.getOrgId() == -1 || SubmitterContactList.size() == 0) { %>
      <%= SubmitterContactList.getEmptyHtmlSelect(systemStatus, "submitterContactId") %>
      <%} else {%>
      <%= SubmitterContactList.getHtmlSelect("submitterContactId", User.getContact().getId()) %>
      <%}%>
      <font
          color="red">*</font><%= showAttribute(request, "submitterContactIdError") %>
      <dhv:permission name="accounts-accounts-contacts-add">
      ...
      <table cellspacing="0" cellpadding="0" border="0" class="empty">
        <tr>
          ...
          <td>
            ...
            <% if (User.getUserRecord().isPortalUser() && isReseller) { %>
            [<a href="javascript:selectServiceContract2();"><dhv:label
              name="accounts.accounts_add.select">Select</dhv:label></a>]
            &nbsp [<a
              href="javascript:changeDivContent('addServiceContract','');javascript:resetNumericFieldValue('contractId');javascript:changeDivContent('addAsset','');javascript:resetNumericFieldValue('assetId');"><dhv:label
              name="button.clear">Clear</dhv:label></a>]
            <% } else { %>
            [<a href="javascript:selectServiceContract();"><dhv:label
              name="accounts.accounts_add.select">Select</dhv:label></a>]
            &nbsp [<a
      ...
      <table cellspacing="0" cellpadding="0" border="0" class="empty">
        <tr>
          ...
          <td>
            ...
            <% if (User.getUserRecord().isPortalUser() && isReseller) { %>[<a
              href="javascript:selectAsset2();"><% } else { %>[<a
              href="javascript:selectAsset();"><% } %><dhv:label
              name="accounts.accounts_add.select">Select</dhv:label></a>]
            &nbsp [<a
...
<dhv:evaluate
    if="<%= (User.getRoleType() == 0) || (User.getUserRecord().isPortalUser()) %>">
  <dhv:include name="ticket.catCode" none="true">
    <tr class="containerBody">
      ...
      <td>
        ...
        <input type="checkbox"
               <dhv:evaluate if="<%= (User.getUserRecord().isPortalUser()) %>">style="display: none" </dhv:evaluate>
               name="autoSetFields" id="autoSetFields" value="true" checked/>
        <dhv:evaluate if="<%= (!User.getUserRecord().isPortalUser()) %>">
          <dhv:label name="tickets.automaticallyPopulateAssignment.text">Automatically
            populate assignment based on categories</dhv:label></dhv:evaluate>
      </td>
    </tr>
  </dhv:include>
