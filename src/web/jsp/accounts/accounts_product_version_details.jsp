...
<a href="AccountsProducts.do?command=Details&orgId=<%=OrgDetails.getOrgId()%>&adId=<%= request.getAttribute("adId") %>&fid=<%= FileItem.getId() %>">Product Details</a> >
Version Details
</td>
</tr>
</table>
...
%>      
    <tr class="row<%= rowid %>">
      <td width="10" align="center" rowspan="2" nowrap>
        <a href="AccountsProducts.do?command=Download&orgId=<%= OrgDetails.getOrgId() %>&adId=<%= request.getAttribute("adId") %>&fid=<%= FileItem.getId() %>&ver=<%= thisVersion.getVersion() %>">Download</a>
      </td>
      <td width="100%">
