...
<form method="post" name="addQuote" action="AccountContactsOppQuotes.do?command=Modify&versionId=<%= version %>&quoteId=<%= quoteBean.getId() %>&auto-populate=true" onSubmit="return checkForm(this);">
<%-- Trails --%>
...
<dhv:container name="accounts" selected="contacts" object="OrgDetails" param="<%= "orgId=" + OrgDetails.getOrgId() %>">
  <dhv:container name="accountscontacts" selected="opportunities" object="ContactDetails" param="<%= "id=" + ContactDetails.getId() %>">
    ...
    <input type="button" value="<dhv:label name="button.cancel">Cancel</dhv:label>" onClick="javascript:window.location.href='AccountContactsOppQuotes.do?command=Details&version=<%= version %>&quoteId=<%= quoteBean.getId() %>&orgId=<%= OrgDetails.getOrgId() %>&contactId=<%= ContactDetails.getId() %>&headerId=<%=quoteBean.getHeaderId() %>';"/>
    <br />
    <dhv:formMessage />
<%
...
%>
    ...
    <br />
    <input type="submit" value="<dhv:label name="global.button.update">Update</dhv:label>" />
    <input type="button" value="<dhv:label name="global.button.cancel">Cancel</dhv:label>" onClick="javascript:window.location.href='AccountContactsOppQuotes.do?command=Details&version=<%= version %>&quoteId=<%= quoteBean.getId() %>&orgId=<%= OrgDetails.getOrgId() %>&contactId=<%= ContactDetails.getId() %>&headerId=<%=quoteBean.getHeaderId() %>';" />
    </dhv:container>
  </dhv:container>
</dhv:container>
</form>
