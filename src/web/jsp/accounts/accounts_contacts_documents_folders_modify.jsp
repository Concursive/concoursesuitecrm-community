<%-- 
  ...
  - 
  - Version: $Id: accounts_contacts_documents_folders_modify.jsp 28.12.2006 17:12:36 zhenya.zhidok $
  - Description:
  --%>
...
<table cellpadding="0" cellspacing="0" width="100%" border="1" rules="cols">
  <tr class="section">
    ...
    <dhv:evaluate if="<%= FileFolder.getParentId() != -1 %>">
      <a href="AccountsContactsDocumentsFolders.do?command=SaveMove&contactId=<%= ContactDetails.getId() %>&id=<%= FileFolder.getId() %>&popup=true&folderId=0&return=AccountsContactsDocuments&param=<%= ContactDetails.getId() %>&param2=<%= FileFolder.getParentId() %>"><dhv:label name="accounts.accounts_documents_file_move.Home">Home</dhv:label></a>
    </dhv:evaluate>
    <dhv:evaluate if="<%= FileFolder.getParentId() == -1 %>">
...
%>    
  <tr class="row<%= rowid %>">
    ...
    <dhv:evaluate if="<%= FileFolder.getParentId() != thisFolder.getId() %>">  
      <a href="AccountsContactsDocumentsFolders.do?command=SaveMove&contactId=<%= ContactDetails.getId() %>&id=<%= FileFolder.getId() %>&popup=true&folderId=<%= thisFolder.getId() %>&return=AccountsContactsDocuments&param=<%= ContactDetails.getId() %>&param2=<%= FileFolder.getParentId() %>"><%= toHtml(thisFolder.getSubject()) %></a>
    </dhv:evaluate>
    <dhv:evaluate if="<%= FileFolder.getParentId() == thisFolder.getId() %>">
