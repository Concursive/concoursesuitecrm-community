<%-- 
  ...
  - 
  - Version: $Id: accounts_contacts_documents_popup_list.jsp 28.12.2006 17:16:57 zhenya.zhidok $
  - Description:
  --%>
...
<dhv:container name="accounts" selected="contacts" object="OrgDetails" param="<%= "orgId=" + OrgDetails.getOrgId() %>" hideContainer="<%= !OrgDetails.getEnabled() || OrgDetails.isTrashed() %>" appendToUrl="<%= addLinkParams(request, "popup|popupType|actionId") %>">
  ...
  <%
   ...
   %>
	...
	%>    
	  <tr>
	    <td class="row<%= rowid %>" width="100%">
	      ...
	      <a href="AccountsContactsDocuments.do?command=Download&contactId=<%= ContactDetails.getId() %>&folderId=<%= thisFile.getFolderId() %>&fid=<%= thisFile.getId() %>"><%= toHtml(thisFile.getSubject()) %></a>
	    </td>
	...
	&nbsp;<br/>
	<form method="post" name="inputForm" action="AccountsContactsDocuments.do?command=Upload&popup=true" onSubmit="return checkFileForm(this);" enctype="multipart/form-data">
