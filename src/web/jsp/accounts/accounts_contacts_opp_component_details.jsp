...
<a href="AccountContactsOppComponents.do?command=ComponentHistory&contactId=<%=contactDetails.getId()%>&headerId=<%= opportunityHeader.getId() %>&id=<%= opportunityComponent.getId() %>"><dhv:label name="accounts.accounts_contacts_oppcomponent.componentLog">Component Log</dhv:label></a> >
...
<dhv:container name="accounts" selected="contacts" object="orgDetails" param="<%= "orgId=" + orgDetails.getOrgId() %>" appendToUrl="<%= addLinkParams(request, "popup|popupType|actionId") %>">
  ...
  <%= addHiddenParams(request, "viewSource|popup|popupType|actionId") %>
