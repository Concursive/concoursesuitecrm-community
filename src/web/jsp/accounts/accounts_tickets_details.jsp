...
<form name="details"
      action="AccountTickets.do?command=ModifyTicket&auto-populate=true<%= addLinkParams(request, "popup|popupType|actionId") %>"
      method="post">
  ...
  <dhv:evaluate if="<%= TicketDetails.isClosed() %>">
    <dhv:permission name="accounts-accounts-tickets-edit">
      <input type="button"
             ...
             onClick="javascript:this.form.action='AccountTickets.do?command=ReopenTicket&id=<%=TicketDetails.getId()%><%= addLinkParams(request, "popup|popupType|actionId") %>';submit();"/>
    </dhv:permission>
  </dhv:evaluate>
  <dhv:evaluate if="<%= !TicketDetails.isClosed() %>">
    <dhv:permission name="accounts-accounts-tickets-edit">
      <input type="button"
             ...
             onClick="javascript:this.form.action='AccountTickets.do?command=ModifyTicket&id=<%=TicketDetails.getId()%><%= addLinkParams(request, "popup|popupType|actionId") %>';submit();"/>
    </dhv:permission>
    <dhv:permission name="accounts-accounts-tickets-delete">
      <input type="button"
             value="<dhv:label name="global.button.delete">Delete</dhv:label>"
             onClick="javascript:popURL('AccountTickets.do?command=ConfirmDelete&orgId=<%= TicketDetails.getOrgId() %>&id=<%= TicketDetails.getId() %>&popup=true<%= isPopup(request)?"&popupType=inline":"" %>', 'Delete_ticket','320','200','yes','no');"/>
    </dhv:permission>
  </dhv:evaluate>
</dhv:evaluate>
<%--
...
--%>
<dhv:permission
    name="accounts-accounts-tickets-edit,accounts-accounts-tickets-delete"><br/>&nbsp;</dhv:permission>
<%-- Primary Contact --%>
<%
  ...
  <tr class="containerBody">
    ...
    <td>
      <dhv:permission name="accounts-accounts-contacts-view">
        <a href="javascript:popURL('Contacts.do?command=Details&id=<%= TicketDetails.getContactId() %>&popup=true','Details','650','500','yes','yes');"><%= toHtml(TicketDetails.getThisContact().getNameFull()) %>
        </a>
      </dhv:permission>
...
<tr class="containerBody">
  ...
  <td>
    ...
    <dhv:evaluate if="<%= !isPortalUserShowLink %>">
      <%= toHtml(SubmitterOrgDetails.getName()) %>
    </dhv:evaluate>
  </td>
</tr>
...
<tr class="containerBody">
  ...
  <td>
    <dhv:evaluate if="<%= isPortalUserShowLink %>">
      <a href="javascript:popURL('Contacts.do?command=Details&id=<%= SubmitterContact.getId() %>&popup=true&popupType=inline','Details','650','500','yes','yes');"><%= toHtml(SubmitterContact.getNameFull()) %>
      </a>
    </dhv:evaluate>
    <dhv:evaluate if="<%= !isPortalUserShowLink %>">
...
<table cellpadding="4" cellspacing="0" border="0" width="100%" class="details">
  ...
  <tr class="containerBody">
    <td class="formLabel" valign="top">
      <dhv:label
          name="accounts.accounts_asset_history.Resolution">Resolution</dhv:label>
    </td>
    <td>
